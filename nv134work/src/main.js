...
handle("admin-login", (_event, payload = {}) => {
  ...
  return { ok: true, username: "admin", version: String(app.getVersion() || "").includes("globalfix") ? "1.3.4" : app.getVersion() };
});
...
handle("admin-status", () => ({ authenticated: adminAuthenticated, server: adminServerStatus, url: adminServerUrl, version: String(app.getVersion() || "").includes("globalfix") ? "1.3.4" : app.getVersion() }));
...
handle("app-version", () => { const v = app.getVersion() || "1.3.4"; return String(v).includes("globalfix") ? "1.3.4" : v; });
