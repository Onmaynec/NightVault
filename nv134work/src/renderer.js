...
function renderAuth() {
  ...
  app.innerHTML = `<div class="auth"><div class="authBox panelIn"><div class="logo">Night<span>Vault</span></div><div class="small">защищённый клиент · SQLite · E2EE · Sync 1.3.4</div>
...
function renderSide() {
  const c = currentChat();
  if (!c)
    return `<div class=sidePad><h2>NightVault</h2>${securitySummary()}<p class=muted>Версия 1.3.4</p></div>`;
  const u =
    c.type === "private"
      ? c.other
      : {
...
/* NightVault 1.3.4 — feature overlay and guarded UX */
const NV120 = {
  version: "1.3.0",
...
/* NightVault 1.3.4 — multi-client/session, E2EE resync and two-panel page fixes */
...
/* NightVault 1.3.4 — renderer hotfixes: links collector, file actions, E2EE key vault UX, voice UI, reactions, menus */
(function nv131Fixed2Layer(){
  const HOTFIX_LABEL = "1.3.4";
...
/* NightVault 1.3.4 — tester fixes: voice player, scroll stability, group avatars, media preview, updater/security polish */
(function nv133Layer(){
  const HOTFIX_LABEL = "1.3.4";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  ...
  window.settingsPanel = settingsPanel = function nv133SettingsPanel(){
    return String(baseSettingsPanel133() || "")
      ...
      .replace(/NightVault [^<]+/g, "NightVault 1.3.4");
  };
  const baseSettingsPage133 = settingsPage;
  window.settingsPage = settingsPage = function nv133SettingsPage(){
    return String(baseSettingsPage133() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.3.4</h1>`);
  };
...
/* NightVault 1.3.4 — Crimson default, Radmin admin hosting, Telegram voice and photo viewer */
(function nv134Layer(){
  const HOTFIX_LABEL = "1.3.4";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  try {
  ...
  function nv134LooksLikeVoice(a, type){
    const name = String(a?.name || "").toLowerCase();
    return type.startsWith("audio/") && (a.voice || Number(a.duration || 0) > 0 || name.startsWith("voice-") || /\.(webm|ogg|opus|m4a|mp3|wav)$/i.test(name));
  }
  ...
  window.settingsPanel = settingsPanel = function nv134SettingsPanel(){
    return String(baseSettingsPanel134() || "").replace(/NightVault [^<]+/g, "NightVault 1.3.4");
  };
  const baseSettingsPage134 = settingsPage;
  window.settingsPage = settingsPage = function nv134SettingsPage(){
    return String(baseSettingsPage134() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.3.4</h1>`).replace(/Настройки 1\.3\.\d+/g, "Настройки 1.3.4");
  };
