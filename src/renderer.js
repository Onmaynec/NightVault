...
function renderAuth() {
  ...
  app.innerHTML = `<div class="auth"><div class="authBox panelIn"><div class="logo">Night<span>Vault</span></div><div class="small">защищённый клиент · SQLite · E2EE · Sync 1.4.0</div>
...
function renderSide() {
  const c = currentChat();
  if (!c)
    return `<div class=sidePad><h2>NightVault</h2>${securitySummary()}<p class=muted>Версия 1.4.0</p></div>`;
  const u =
    c.type === "private"
      ? c.other
      : {
...
/* NightVault 1.4.0 — feature overlay and guarded UX */
const NV120 = {
  version: "1.3.0",
...
/* NightVault 1.4.0 — multi-client/session, E2EE resync and two-panel page fixes */
...
/* NightVault 1.4.0 — renderer hotfixes: links collector, file actions, E2EE key vault UX, voice UI, reactions, menus */
(function nv131Fixed2Layer(){
  const HOTFIX_LABEL = "1.4.0";
...
/* NightVault 1.4.0 — tester fixes: voice player, scroll stability, group avatars, media preview, updater/security polish */
(function nv133Layer(){
  const HOTFIX_LABEL = "1.4.0";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  ...
  window.settingsPanel = settingsPanel = function nv133SettingsPanel(){
    return String(baseSettingsPanel133() || "")
      ...
      .replace(/NightVault [^<]+/g, "NightVault 1.4.0");
  };
  const baseSettingsPage133 = settingsPage;
  window.settingsPage = settingsPage = function nv133SettingsPage(){
    return String(baseSettingsPage133() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.4.0</h1>`);
  };
...
/* NightVault 1.4.0 — Crimson default, Radmin admin hosting, Telegram voice and photo viewer */
(function nv134Layer(){
  const HOTFIX_LABEL = "1.4.0";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  try {
  ...
  window.settingsPanel = settingsPanel = function nv134SettingsPanel(){
    return String(baseSettingsPanel134() || "").replace(/NightVault [^<]+/g, "NightVault 1.4.0");
  };
  const baseSettingsPage134 = settingsPage;
  window.settingsPage = settingsPage = function nv134SettingsPage(){
    return String(baseSettingsPage134() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.4.0</h1>`).replace(/Настройки 1\.3\.\d+/g, "Настройки 1.4.0");
  };
...
/* NightVault 1.4.0 — live sync, styled group modals, real voice UI, contact notifications, group avatar */
(function nv135Layer(){
  const HOTFIX_LABEL = "1.4.0";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  ...
  window.settingsPanel = settingsPanel = function nv135SettingsPanel(){ return String(baseSettingsPanel135() || "").replace(/NightVault [^<]+/g, "NightVault 1.4.0"); };
  const baseSettingsPage135 = settingsPage;
  window.settingsPage = settingsPage = function nv135SettingsPage(){ return String(baseSettingsPage135() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.4.0</h1>`).replace(/Настройки 1\.3\.\d+/g, "Настройки 1.4.0"); };
})();

/* NightVault 1.4.0 — quiet onboarding, stable live rendering, profile/banner/photo/voice fixes */
(function nv136Layer(){
  const HOTFIX_LABEL = "1.4.0";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  ...
  window.settingsPanel = settingsPanel = function nv136SettingsPanel() {
    return String(baseSettingsPanel136() || "").replace(/NightVault 1\.2\.0 Mega Release/g, "NightVault 1.4.0").replace(/Messenger Features Update:/g, "NightVault 1.4.0:").replace(/1\.3\.5/g, "1.4.0");
  };
  const baseSettingsPage136 = settingsPage;
  window.settingsPage = settingsPage = function nv136SettingsPage() {
    return String(baseSettingsPage136() || "").replace(/1\.3\.5/g, "1.4.0").replace(/Messenger Features Update:/g, "NightVault 1.4.0:");
  };
})();

/* NightVault 1.4.0 — Messenger Features Update: partial render, WS-first live events, custom voice, media viewer */
(function nv137PerformanceUxLayer(){
  ...
  window.settingsPanel = settingsPanel = function nv137SettingsPanel() {
    return String(baseSettingsPanel137() || "").replace(/NightVault 1\.2\.0 Mega Release/g, "NightVault 1.4.0 Messenger Features Update").replace(/Mega Release:/g, "Messenger Features Update:").replace(/1\.3\.[0-6]/g, "1.4.0");
  };
  const baseSettingsPage137 = settingsPage;
  window.settingsPage = settingsPage = function nv137SettingsPage() {
    return String(baseSettingsPage137() || "").replace(/NightVault 1\.2\.0 Mega Release/g, "NightVault 1.4.0 Messenger Features Update").replace(/Mega Release:/g, "Messenger Features Update:").replace(/1\.3\.[0-6]/g, "1.4.0");
  };
})();

/* NightVault 1.4.0 — server admin events from Admin Pro */
...
/* NightVault 1.4.0 — Messenger Features Update: forward, saved messages, media history, folders, mentions, notifications, paste/upload UX */
(function nv139MessengerFeaturesLayer(){
  ...
  window.contactsPage = contactsPage = function nv139ContactsPage(){ return String(baseContactsPage139()||'').replace('1.0.9: отдельная система заявок, избранных, заметок и приватности “только контакты”.', 'Messenger Features 1.4.0: заявки, избранные, блокировки, уведомления и быстрый поиск.'); };
  ...
  window.settingsPanel = settingsPanel = function nv139SettingsPanel(){
    ...
    return String(baseSettingsPanel139()||'').replace(/NightVault 1\.3\.\d+/g,'NightVault 1.4.0').replace(/Admin Pro & Server Control Update/g,'Messenger Features Update');
  };
  const baseSettingsPage139 = settingsPage;
  window.settingsPage = settingsPage = function nv139SettingsPage(){ return String(baseSettingsPage139()||'').replace(/1\.3\.\d+/g,'1.4.0').replace(/Admin Pro & Server Control Update/g,'Messenger Features Update'); };
})();


/* NightVault 1.4.0 — Security & Messenger Pro Update: UI spacing, safe avatars, context menu bounds, E2EE cache and pro features */
(() => {
  const VERSION = "1.4.0";
  try { localStorage.nvAppVersion = VERSION; } catch {}
  ...
  function nv140HashPin(pin, salt) { return crypto.subtle.digest("SHA-256", new TextEncoder().encode(`${salt}:${pin}:NightVault-1.4.0`)).then((buf)=>Array.from(new Uint8Array(buf)).map((b)=>b.toString(16).padStart(2,"0")).join("")); }
  ...
  window.settingsPanel = settingsPanel = function nv140SettingsPanel(){
    ...
    if (section === "developer") return `<section class="settingsPanel"><h2>Диагностика 1.4.0</h2><p class="muted">Crash Report 2.0, performance-audit, security-audit и debug bundle.</p><div class="buttonRow"><button class="btn" onclick="nv120CollectDebugReport()">Crash report</button><button class="btn ghost" onclick="nv140SecurityAudit()">Security audit</button><button class="btn ghost" onclick="toast('Performance audit запускается командой npm run perf-audit')">Performance audit</button></div><textarea id="debugReport" class="field" rows="8" placeholder="Здесь появится debug report"></textarea></section>`;
  ...
  window.showChangelogModal = showChangelogModal = function nv140ChangelogModal(data={}){
    ...
    setTimeout(()=>modal(`<div class="updateHero nv140Changelog"><div class="updateGlow">1.4</div><h2>${h(data.title || 'NightVault 1.4.0 — Security & Messenger Pro Update')}</h2><div class="fileCard"><b>Версия ${h(data.version || VERSION)}</b></div><ul class="changeList">${changes || '<li>Добавлены Security & Messenger Pro функции и исправления интерфейса.</li>'}</ul><button class="btn" onclick="closeModal()">Понятно</button></div>`),700);
  };
})();
