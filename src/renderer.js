...
const defaultState = () => ({
  ...
  theme: localStorage.nvTheme || "aurora",
  accent: localStorage.nvAccent || "#8b5cf6",
  blur: localStorage.nvBlur === "1",
  selected: new Set(),
  editId: null,
  micId: localStorage.nvMicId || "",
  accounts: [],
  chatBg: (localStorage.nvChatBg === "frost" ? "none" : (localStorage.nvChatBg || "aurora")),
...
function renderAuth() {
  ...
  app.innerHTML = `<div class="auth"><div class="authBox panelIn"><div class="logo">Night<span>Vault</span></div><div class="small">защищённый клиент · SQLite · E2EE · Sync 1.3.3</div>
...
function renderSide() {
  const c = currentChat();
  if (!c)
    return `<div class=sidePad><h2>NightVault</h2>${securitySummary()}<p class=muted>Версия 1.3.3</p></div>`;
  const u =
    c.type === "private"
      ? c.other
      : {
...
/* NightVault 1.3.3 — feature overlay and guarded UX */
const NV120 = {
  version: "1.3.0",
...
/* NightVault 1.3.3 — multi-client/session, E2EE resync and two-panel page fixes */
...
/* NightVault 1.3.3 — renderer hotfixes: links collector, file actions, E2EE key vault UX, voice UI, reactions, menus */
(function nv131Fixed2Layer(){
  const HOTFIX_LABEL = "1.3.3";
...
/* NightVault 1.3.3 — tester fixes: voice player, scroll stability, group avatars, media preview, updater/security polish */
(function nv133Layer(){
  const HOTFIX_LABEL = "1.3.3";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  ...
  window.settingsPanel = settingsPanel = function nv133SettingsPanel(){
    return String(baseSettingsPanel133() || "")
      ...
      .replace(/NightVault [^<]+/g, "NightVault 1.3.3");
  };
  const baseSettingsPage133 = settingsPage;
  window.settingsPage = settingsPage = function nv133SettingsPage(){
    return String(baseSettingsPage133() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.3.3</h1>`);
  };
