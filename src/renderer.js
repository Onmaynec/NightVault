...
function renderAuth() {
  ...
  app.innerHTML = `<div class="auth"><div class="authBox panelIn"><div class="logo">Night<span>Vault</span></div><div class="small">защищённый клиент · SQLite · E2EE · Sync 1.4.4</div>
...
function renderSide() {
  const c = currentChat();
  if (!c)
    return `<div class=sidePad><h2>NightVault</h2>${securitySummary()}<p class=muted>Версия 1.4.4</p></div>`;
  const u =
    c.type === "private"
      ? c.other
      : {
...
/* NightVault 1.4.4 — feature overlay and guarded UX */
const NV120 = {
  version: "1.3.0",
...
/* NightVault 1.4.4 — multi-client/session, E2EE resync and two-panel page fixes */
...
/* NightVault 1.4.4 — renderer hotfixes: links collector, file actions, E2EE key vault UX, voice UI, reactions, menus */
(function nv131Fixed2Layer(){
  const HOTFIX_LABEL = "1.4.4";
...
/* NightVault 1.4.4 — tester fixes: voice player, scroll stability, group avatars, media preview, updater/security polish */
(function nv133Layer(){
  const HOTFIX_LABEL = "1.4.4";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  ...
  window.settingsPanel = settingsPanel = function nv133SettingsPanel(){
    return String(baseSettingsPanel133() || "")
      ...
      .replace(/NightVault [^<]+/g, "NightVault 1.4.4");
  };
  const baseSettingsPage133 = settingsPage;
  window.settingsPage = settingsPage = function nv133SettingsPage(){
    return String(baseSettingsPage133() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.4.4</h1>`);
  };
...
/* NightVault 1.4.4 — Crimson default, Radmin admin hosting, Telegram voice and photo viewer */
(function nv134Layer(){
  const HOTFIX_LABEL = "1.4.4";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  try {
  ...
  window.settingsPanel = settingsPanel = function nv134SettingsPanel(){
    return String(baseSettingsPanel134() || "").replace(/NightVault [^<]+/g, "NightVault 1.4.4");
  };
  const baseSettingsPage134 = settingsPage;
  window.settingsPage = settingsPage = function nv134SettingsPage(){
    return String(baseSettingsPage134() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.4.4</h1>`).replace(/Настройки 1\.3\.\d+/g, "Настройки 1.4.4");
  };
...
/* NightVault 1.4.4 — live sync, styled group modals, real voice UI, contact notifications, group avatar */
(function nv135Layer(){
  const HOTFIX_LABEL = "1.4.4";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  ...
  window.settingsPanel = settingsPanel = function nv135SettingsPanel(){ return String(baseSettingsPanel135() || "").replace(/NightVault [^<]+/g, "NightVault 1.4.4"); };
  const baseSettingsPage135 = settingsPage;
  window.settingsPage = settingsPage = function nv135SettingsPage(){ return String(baseSettingsPage135() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.4.4</h1>`).replace(/Настройки 1\.3\.\d+/g, "Настройки 1.4.4"); };
})();

/* NightVault 1.4.4 — quiet onboarding, stable live rendering, profile/banner/photo/voice fixes */
(function nv136Layer(){
  const HOTFIX_LABEL = "1.4.4";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  ...
  window.settingsPanel = settingsPanel = function nv136SettingsPanel() {
    return String(baseSettingsPanel136() || "").replace(/NightVault 1\.2\.0 Mega Release/g, "NightVault 1.4.4").replace(/Messenger Features Update:/g, "NightVault 1.4.4:").replace(/1\.3\.5/g, "1.4.4");
  };
  const baseSettingsPage136 = settingsPage;
  window.settingsPage = settingsPage = function nv136SettingsPage() {
    return String(baseSettingsPage136() || "").replace(/1\.3\.5/g, "1.4.4").replace(/Messenger Features Update:/g, "NightVault 1.4.4:");
  };
})();

/* NightVault 1.4.4 — Messenger Features Update: partial render, WS-first live events, custom voice, media viewer */
(function nv137PerformanceUxLayer(){
  ...
  window.settingsPanel = settingsPanel = function nv137SettingsPanel() {
    return String(baseSettingsPanel137() || "").replace(/NightVault 1\.2\.0 Mega Release/g, "NightVault 1.4.4 Messenger Features Update").replace(/Mega Release:/g, "Messenger Features Update:").replace(/1\.3\.[0-6]/g, "1.4.4");
  };
  const baseSettingsPage137 = settingsPage;
  window.settingsPage = settingsPage = function nv137SettingsPage() {
    return String(baseSettingsPage137() || "").replace(/NightVault 1\.2\.0 Mega Release/g, "NightVault 1.4.4 Messenger Features Update").replace(/Mega Release:/g, "Messenger Features Update:").replace(/1\.3\.[0-6]/g, "1.4.4");
  };
})();

/* NightVault 1.4.4 — server admin events from Admin Pro */
...
/* NightVault 1.4.4 — Messenger Features Update: forward, saved messages, media history, folders, mentions, notifications, paste/upload UX */
(function nv139MessengerFeaturesLayer(){
  ...
  window.contactsPage = contactsPage = function nv139ContactsPage(){ return String(baseContactsPage139()||'').replace('1.0.9: отдельная система заявок, избранных, заметок и приватности “только контакты”.', 'Messenger Features 1.4.4: заявки, избранные, блокировки, уведомления и быстрый поиск.'); };
  ...
  window.settingsPanel = settingsPanel = function nv139SettingsPanel(){
    ...
    return String(baseSettingsPanel139()||'').replace(/NightVault 1\.3\.\d+/g,'NightVault 1.4.4').replace(/Admin Pro & Server Control Update/g,'Messenger Features Update');
  };
  const baseSettingsPage139 = settingsPage;
  window.settingsPage = settingsPage = function nv139SettingsPage(){ return String(baseSettingsPage139()||'').replace(/1\.3\.\d+/g,'1.4.4').replace(/Admin Pro & Server Control Update/g,'Messenger Features Update'); };
})();


/* NightVault 1.4.4 — Security & Messenger Pro Update: UI spacing, safe avatars, context menu bounds, E2EE cache and pro features */
(() => {
  const VERSION = "1.4.4";
  try { localStorage.nvAppVersion = VERSION; } catch {}
  if (typeof S === "object") S.appVersion = VERSION;

  function nv140JsonLoad(key, fallback) { try { return JSON.parse(localStorage[key] || ""); } catch { return fallback; } }
  function nv140JsonSave(key, value) { try { localStorage[key] = JSON.stringify(value); } catch {} }
  function nv140CacheKey(message) { return `nv140Decrypted_${S.user?.username || "guest"}_${message?.id || ""}`; }
  function nv140SaveDecryptedCache(message) {
    if (!message?.id || !message.decrypted) return;
    const payload = { text: message.decryptedText || "", attachment: message.decryptedAttachment || null, at: Date.now() };
    nv140JsonSave(nv140CacheKey(message), payload);
  }
  function nv140ApplyDecryptedCache(message) {
    if (!message?.id || !message.decryptError) return message;
    const cached = nv140JsonLoad(nv140CacheKey(message), null);
    if (!cached) return message;
    message.decrypted = true;
    message.decryptError = false;
    message.decryptedText = cached.text || "";
    if (cached.attachment) message.decryptedAttachment = cached.attachment;
    message.decryptHint = "local_recovery_cache";
    return message;
  }
  const baseDecrypt140 = decryptMessage;
  window.decryptMessage = decryptMessage = async function nv140DecryptMessage(message) {
    const result = await baseDecrypt140(message);
    if (result?.decrypted) nv140SaveDecryptedCache(result);
    if (result?.decryptError) nv140ApplyDecryptedCache(result);
    return result;
  };

  window.nv140AvatarFallback = function nv140AvatarFallback(img) {
    const name = img?.getAttribute?.("data-name") || img?.getAttribute?.("data-fallback") || "?";
    const cls = img?.className || "avatar";
    const div = document.createElement("div");
    div.className = cls;
    div.textContent = String(name || "?").slice(0, 2).toUpperCase();
    div.style.cssText = img?.getAttribute?.("style") || "";
    img?.replaceWith(div);
  };
  const baseAv140 = av;
  window.av = av = function nv140Avatar(u, cls = "avatar") {
    const name = String(u?.displayName || u?.username || "?").replace(/Медиа|Мед/g, "");
    const html = String(baseAv140({ ...(u || {}), displayName: name || u?.displayName, username: u?.username || name }, cls) || "");
    if (/^<img\b/i.test(html)) return html.replace(/<img\b/, `<img data-name="${h(name)}" onerror="nv140AvatarFallback(this)"`).replace(/alt="[^"]*"/, `alt="${h(name)}"`);
    return html;
  };

  function nv140MentionTextHtml(raw = "", query = "") {
    let text = h(raw);
    text = text.replace(/(^|\s)(@(all|admins|[\wа-яё-]{2,32}))/giu, (m, pre, mention) => `${pre}<span class="mentionChip">${h(mention)}</span>`);
    query = String(query || "").slice(0, 64);
    if (query) {
      const re = new RegExp("(" + escapeReg(query) + ")", "ig");
      text = text.replace(re, '<mark class="highlight">$1</mark>');
    ...
    text = text
      ...
      .replace(/`([^`]{1,240})`/g, "<code>$1</code>")
      .replace(/^&gt;\s?(.+)$/gm, '<blockquote>$1</blockquote>');
    return text;
  }
  window.messageTextHtml = messageTextHtml = nv140MentionTextHtml;

  async function nv140UploadRawImage(file) {
    if (!(file instanceof Blob)) throw new Error("Некорректный файл аватара.");
    const name = String(file.name || "avatar.png");
    if (!String(file.type || "").startsWith("image/") && !/\.(png|jpe?g|webp|gif)$/i.test(name)) throw new Error("Для аватарки группы нужен PNG/JPEG/WebP/GIF.");
    if (file.size > 8 * 1024 * 1024) throw new Error("Аватарка группы больше 8 MB.");
    const form = new FormData();
    form.append("file", file, name);
    return await api("/files", { method:"POST", body:form, timeout:120000 });
  }
  window.nv135PickGroupAvatar = async function nv140PickGroupAvatar() {
    try {
      const files = await pickDomFiles({ accept: "image/png,image/jpeg,image/webp,image/gif", multiple:false });
      const file = files?.[0]; if (!file) return;
      const uploaded = await nv140UploadRawImage(file);
      window.nv135GroupAvatarUploaded = uploaded;
  ...
  function nv140BoundMenu(node, x, y, padding = 12) {
    document.body.appendChild(node);
    window.NVActionBridge?.bind?.(node);
    requestAnimationFrame(() => {
      const rect = node.getBoundingClientRect();
      const maxW = Math.min(360, window.innerWidth - padding * 2);
      node.style.maxWidth = `${maxW}px`;
      const left = Math.max(padding, Math.min(x, window.innerWidth - Math.min(rect.width, maxW) - padding));
      const top = Math.max(padding, Math.min(y, window.innerHeight - Math.min(rect.height, window.innerHeight - padding * 2) - padding));
      node.style.left = `${left}px`; node.style.top = `${top}px`;
      node.style.maxHeight = `${Math.max(180, window.innerHeight - top - padding)}px`;
      node.style.overflowY = "auto";
    });
  }
  window.ctx = ctx = function nv140Ctx(e, id) {
    e.preventDefault(); e.stopPropagation?.();
    document.querySelector(".ctx")?.remove();
    const message = findMsg(id); const mine = message?.from === S.user?.username;
    const d = document.createElement("div"); d.className = "ctx ctxRich ctxSafe nv140Ctx";
    const quick = reactionList.map((emoji) => `<button class="reactQuick" onclick="react('${id}','${inlineArg(emoji)}')">${h(emoji)}</button>`).join("");
    d.innerHTML = `<button class="ctxClose" onclick="document.querySelector('.ctx')?.remove()">×</button><div class=ctxTitle>Реакция</div><div class=ctxReactions>${quick}</div><button onclick="S.replyTo='${id}';document.querySelector('.ctx')?.remove();render()">Ответить</button><button onclick="toggleSelect('${id}');document.querySelector('.ctx')?.remove();renderMessagesOnly(false)">Выделить</button>${mine ? `<button onclick="startEdit('${id}')">Редактировать</button><button onclick="delMsg('${id}',1)">Удалить у всех</button>` : ""}<button onclick="pinMsg('${id}')">Закрепить</button><button onclick="delMsg('${id}',0)">Удалить у себя</button><button onclick="nv139ForwardMessage('${id}')">Переслать</button><button onclick="nv139SaveMessage('${id}')">Сохранить в Избранное</button><button onclick="nv139ReportMessage('${id}')">Пожаловаться</button>`;
    nv140BoundMenu(d, e.clientX, e.clientY);
  };

  const baseForwardOne140 = typeof forwardOne139 === "function" ? forwardOne139 : null;
  if (baseForwardOne140) {
    window.forwardOne139 = forwardOne139 = async function nv140ForwardOne(targetChatId, m) {
      const a = messagePayloadAttachment(m);
      const text = messagePayloadText(m);
      const label = m.from && m.from !== S.user?.username ? `Переслано от @${m.from}\n` : "";
      const payload = await buildOutgoingMessagePayload(targetChatId, label + text, a || null);
  ...
  function nv140HashPin(pin, salt) { return crypto.subtle.digest("SHA-256", new TextEncoder().encode(`${salt}:${pin}:NightVault-1.4.4`)).then((buf)=>Array.from(new Uint8Array(buf)).map((b)=>b.toString(16).padStart(2,"0")).join("")); }
  window.nv140PinSetup = async function nv140PinSetup() {
    const pin = prompt("Новый PIN для локальной блокировки:");
    if (!pin || pin.length < 4) return toast("PIN должен быть минимум 4 символа.");
    const salt = crypto.getRandomValues(new Uint8Array(16));
    const saltHex = Array.from(salt).map((b)=>b.toString(16).padStart(2,"0")).join("");
    const hash = await nv140HashPin(pin, saltHex);
    nv140JsonSave("nv140Pin", { salt:saltHex, hash, autoLock:Number(localStorage.nv140AutoLock || 5) });
    toast("Локальная блокировка включена");
  };
  window.nv140LockNow = function nv140LockNow(){ document.body.classList.add("nv140Locked"); modal(`<div class="nv140Lock"><h2>NightVault заблокирован</h2><input id="nv140PinInput" class="field" type="password" placeholder="Введите PIN"><button class="btn" onclick="nv140Unlock()">Разблокировать</button></div>`, { lock:true }); setTimeout(()=>document.getElementById('nv140PinInput')?.focus(),80); };
  window.nv140Unlock = async function nv140Unlock(){ const data=nv140JsonLoad("nv140Pin", null); const value=document.getElementById("nv140PinInput")?.value || ""; if(!data) return closeModal(); const hash=await nv140HashPin(value, data.salt); if(hash===data.hash){ document.body.classList.remove("nv140Locked"); closeModal(); } else toast("Неверный PIN"); };
  window.addEventListener("blur", () => { if (localStorage.nv140BlurLock === "1") document.body.classList.add("nv140PrivacyBlur"); });
  window.addEventListener("focus", () => document.body.classList.remove("nv140PrivacyBlur"));

  window.nv140SafetyNumber = async function nv140SafetyNumber(username="") {
    try { const data = username ? await api(`/e2ee/fingerprint/${encodeURIComponent(username)}`) : { username:S.user?.username, devices:Object.values(S.user?.e2eeDevices||{}) }; modal(`<h2>Safety Number</h2><p class="muted">Проверь fingerprint с собеседником перед важной перепиской.</p>${(data.devices||[]).map((d)=>`<div class="fileCard"><b>${h(d.device||d.id||'device')}</b><code>${h(d.fingerprint||d.id||'')}</code></div>`).join('') || '<p class="muted">Устройств пока нет.</p>'}<button class="btn" onclick="closeModal()">Готово</button>`); } catch(e){ toast("Safety number недоступен: "+(e.message||e)); }
  };
  window.nv140SecurityAudit = async function nv140SecurityAudit(){ try{ const r=await api('/security/audit'); modal(`<h2>Security Audit</h2><pre class="testResult">${h(JSON.stringify(r,null,2))}</pre>`); }catch(e){ toast('Security audit: '+(e.message||e)); } };

  const baseSettingsPanel140 = settingsPanel;
  window.settingsPanel = settingsPanel = function nv140SettingsPanel(){
    const section = localStorage.nvSettingsSection || "overview";
    if (section === "privacy") return `<section class="settingsPanel nv140SecurityPanel"><h2>Безопасность и приватность 1.4.4</h2><p class="muted">PIN, blur-lock, 2FA, recovery, доверенные устройства, блокировки и privacy settings.</p><div class="settingCards"><div class="settingCard"><b>Local PIN Lock</b><span>${nv140JsonLoad('nv140Pin',null)?'включён':'выключен'}</span><small>локальная защита приложения</small></div><div class="settingCard"><b>E2EE Trust</b><span>${h(S.e2ee?.deviceId||'device')}</span><small>Safety number / fingerprint</small></div><div class="settingCard"><b>Privacy</b><span>${h(S.user?.privacy?.avatar||'all')}</span><small>аватар, баннер, online, last seen</small></div></div><div class="buttonRow"><button class="btn" onclick="nv140PinSetup()">Включить / сменить PIN</button><button class="btn ghost" onclick="nv140LockNow()">Заблокировать сейчас</button><button class="btn ghost" onclick="localStorage.nv140BlurLock=localStorage.nv140BlurLock==='1'?'0':'1';toast('Blur lock: '+(localStorage.nv140BlurLock==='1'?'on':'off'))">Blur при потере фокуса</button><button class="btn ghost" onclick="nv140SafetyNumber()">Safety number</button><button class="btn ghost" onclick="nv140SecurityAudit()">Security audit</button></div><div class="securityCard"><b>Recovery 2.0</b><br>Используй экспорт recovery key и backup перед переустановкой. Сервер не хранит plain-ключи.</div></section>`;
    if (section === "devices") return `<section class="settingsPanel"><h2>Мои устройства и сессии</h2><p class="muted">Trust Devices UI 2.0, safety number, E2EE key status и отключение сессий.</p><div class="buttonRow"><button class="btn" onclick="devices()">Открыть список устройств</button><button class="btn ghost" onclick="nv140SafetyNumber()">Показать fingerprint</button><button class="btn ghost" onclick="nv131RegisterCurrentDevice(false)">Пересинхронизировать E2EE</button><button class="btn ghost" onclick="nv120ExportRecovery()">Экспорт recovery</button></div><div class="securityCard">Device ID: <code>${h(S.e2ee?.deviceId||'создаётся')}</code></div></section>`;
    if (section === "developer") return `<section class="settingsPanel"><h2>Диагностика 1.4.4</h2><p class="muted">Crash Report 2.0, performance-audit, security-audit и debug bundle.</p><div class="buttonRow"><button class="btn" onclick="nv120CollectDebugReport()">Crash report</button><button class="btn ghost" onclick="nv140SecurityAudit()">Security audit</button><button class="btn ghost" onclick="toast('Performance audit запускается командой npm run perf-audit')">Performance audit</button></div><textarea id="debugReport" class="field" rows="8" placeholder="Здесь появится debug report"></textarea></section>`;
    return String(baseSettingsPanel140()||"").replace(/1\.3\.9/g, VERSION).replace(/Messenger Features Update/g, "Security & Messenger Pro Update");
  };
  const baseSettingsPage140 = settingsPage;
  window.settingsPage = settingsPage = function nv140SettingsPage(){ return String(baseSettingsPage140()||"").replace(/1\.3\.9/g, VERSION).replace(/Messenger Features Update/g, "Security & Messenger Pro Update"); };

  window.showChangelogModal = showChangelogModal = function nv140ChangelogModal(data={}){
    const changes = (data.changes || data.items || data.highlights || []).map((x)=>`<li>${h(x)}</li>`).join("");
    setTimeout(()=>modal(`<div class="updateHero nv140Changelog"><div class="updateGlow">1.4</div><h2>${h(data.title || 'NightVault 1.4.4 — Security & Messenger Pro Update')}</h2><div class="fileCard"><b>Версия ${h(data.version || VERSION)}</b></div><ul class="changeList">${changes || '<li>Добавлены Security & Messenger Pro функции и исправления интерфейса.</li>'}</ul><button class="btn" onclick="closeModal()">Понятно</button></div>`),700);
  };
})();

/* NightVault 1.4.4 — Stability, UX Polish & E2EE Recovery Fix Update */
(function nv141StabilityUxPatch(){
  const VERSION = "1.4.4";
  try { localStorage.nvAppVersion = VERSION; } catch {}
  if (typeof S === "object") S.appVersion = VERSION;

  function nv141JsonLoad(key, fallback) { try { return JSON.parse(localStorage[key] || ""); } catch { return fallback; } }
  function nv141JsonSave(key, value) { try { localStorage[key] = JSON.stringify(value); } catch {} }
  function nv141FileIdFrom(value) {
    const match = String(value || "").match(/\/api\/files\/([a-f0-9-]{18,64})/i);
    return match ? match[1] : "";
  }
  function nv141AttachmentForForward(attachment) {
    if (!attachment) return null;
    const id = attachment.id || nv141FileIdFrom(attachment.url) || nv141FileIdFrom(attachment.previewUrl);
    return { ...attachment, id, url: attachment.url || (id ? `/api/files/${id}` : attachment.url) };
  }

  window.closeEmojiPanel = function closeEmojiPanel(){ document.querySelectorAll(".emojiPanel,.emojiPanelRich,.nv131EmojiPanel").forEach((node)=>node.remove()); };
  const baseToggleEmoji141 = typeof toggleEmojiPicker === "function" ? toggleEmojiPicker : null;
  window.toggleEmojiPicker = toggleEmojiPicker = function nv141ToggleEmojiPicker(){
    if (document.querySelector(".emojiPanel")) return closeEmojiPanel();
    if (baseToggleEmoji141) baseToggleEmoji141();
    const panel = document.querySelector(".emojiPanel");
    if (panel && !panel.querySelector(".emojiClose")) panel.insertAdjacentHTML("afterbegin", `<button class="emojiClose" onclick="closeEmojiPanel()">×</button>`);
    else panel?.querySelector(".emojiClose")?.setAttribute("onclick", "closeEmojiPanel()");
    window.NVActionBridge?.bind?.(panel || document.body);
    requestAnimationFrame(()=>{
      const node = document.querySelector(".emojiPanel");
      if (!node) return;
      const rect = node.getBoundingClientRect();
      const pad = 12;
      node.style.left = Math.max(pad, Math.min(rect.left, window.innerWidth - rect.width - pad)) + "px";
      node.style.top = Math.max(pad, Math.min(rect.top, window.innerHeight - Math.min(rect.height, window.innerHeight-pad*2) - pad)) + "px";
      node.style.maxHeight = Math.max(220, window.innerHeight - Number.parseFloat(node.style.top || rect.top) - pad) + "px";
    });
  };

  // Microphone selector recovery: always expose a visible selector and refresh action in notification settings.
  async function nv141LoadAudioDevicesWithPermission() {
    try {
      if (!navigator.mediaDevices?.enumerateDevices) throw new Error("MediaDevices API недоступен");
      let devices = await navigator.mediaDevices.enumerateDevices();
      let inputs = devices.filter((d)=>d.kind === "audioinput");
      if (!inputs.length && navigator.mediaDevices.getUserMedia) {
        const stream = await navigator.mediaDevices.getUserMedia({ audio:true });
        stream.getTracks().forEach((track)=>track.stop());
        devices = await navigator.mediaDevices.enumerateDevices();
        inputs = devices.filter((d)=>d.kind === "audioinput");
      }
      S.audioDevices = inputs;
      return inputs;
    } catch (error) {
      S.audioDevices = [];
      throw error;
  ...
  window.nv141RefreshMicrophones = async function nv141RefreshMicrophones(){
    try {
      const items = await nv141LoadAudioDevicesWithPermission();
      toast(items.length ? `Микрофоны найдены: ${items.length}` : "Микрофоны не найдены");
      if (S.tab === "settings") render();
    } catch (error) { toast("Нет доступа к микрофону: " + (error.message || error)); }
  };
  const baseSettingsPanel141 = settingsPanel;
  window.settingsPanel = settingsPanel = function nv141SettingsPanel(){
    const section = S.settingsSection || localStorage.nvSettingsSection || "overview";
    if (section === "notifications") {
      if (!S.audioDevices || !S.audioDevices.length) setTimeout(()=>loadAudioDevices?.().then(()=>{ if((S.settingsSection||localStorage.nvSettingsSection)==="notifications") render(); }).catch(()=>{}), 60);
      const options = [`<option value="">Системный микрофон</option>`].concat((S.audioDevices || []).map((device, index)=>`<option value="${h(device.deviceId)}" ${S.micId===device.deviceId?'selected':''}>${h(device.label || `Микрофон ${index+1}`)}</option>`)).join("");
      return `<section class="settingsPanel"><h2>Уведомления и звук</h2><p class="muted">Звук, toast, mute и выбор микрофона для голосовых сообщений.</p><label class="toggleRow"><input id="notify" type="checkbox" ${S.settings.notify !== false ? "checked" : ""}> <span>Показывать системные уведомления</span></label><label class="toggleRow"><input id="messageSound" type="checkbox" ${localStorage.nvMessageSound !== "0" ? "checked" : ""}> <span>Звук нового сообщения</span></label><label class="toggleRow"><input id="toastPreview" type="checkbox" ${localStorage.nvToastPreview !== "0" ? "checked" : ""}> <span>Показывать текст сообщения в toast</span></label><div class="nv141MicBox"><label>Микрофон<select id="mic" class="field">${options}</select></label><div class="buttonRow"><button class="btn ghost" onclick="nv141RefreshMicrophones()">Обновить список микрофонов</button><button class="btn ghost" onclick="testMicList()">Проверить доступ</button></div><small class="muted">Выбор сохраняется локально и используется при записи голосовых.</small></div><button class="btn" onclick="saveSettings()">Сохранить уведомления</button></section>`;
    }
    return String(baseSettingsPanel141() || "").replace(/1\.4\.0/g, VERSION);
  };

  const baseSaveSettings141 = saveSettings;
  window.saveSettings = saveSettings = async function nv141SaveSettings(){
    const mic = document.getElementById("mic");
    if (mic) { S.micId = mic.value || ""; localStorage.nvMicId = S.micId; }
    return baseSaveSettings141();
  };
  const baseStartVoice141 = startVoice;
  window.startVoice = startVoice = async function nv141StartVoice(){
    const mic = document.getElementById("mic");
    if (mic) { S.micId = mic.value || ""; localStorage.nvMicId = S.micId; }
    return baseStartVoice141();
  };

  // E2EE local recovery cache 2.0: save useful decrypted payloads and reuse after restart if device keys are not enough.
  function nv141CacheKey(message) { return `nv141Decrypted_${S.user?.username || "guest"}_${message?.id || ""}`; }
  function nv141SaveDecrypted(message) {
    if (!message?.id || !message.decrypted) return;
    nv141JsonSave(nv141CacheKey(message), { text: message.decryptedText || message.text || "", attachment: message.decryptedAttachment || null, at: Date.now(), chatId: message.chatId || "" });
  }
  function nv141RestoreDecrypted(message) {
    if (!message?.id || !message.decryptError) return message;
    const cached = nv141JsonLoad(nv141CacheKey(message), null) || nv141JsonLoad(`nv140Decrypted_${S.user?.username || "guest"}_${message?.id || ""}`, null);
    if (!cached) return message;
    message.decrypted = true;
    message.decryptError = false;
    message.decryptedText = cached.text || "";
    if (cached.attachment) message.decryptedAttachment = cached.attachment;
    message.decryptHint = "local_recovery_cache_2";
    return message;
  }
  const baseDecrypt141 = decryptMessage;
  window.decryptMessage = decryptMessage = async function nv141DecryptMessage(message){
    const result = await baseDecrypt141(message);
    if (result?.decrypted) nv141SaveDecrypted(result);
    if (result?.decryptError) nv141RestoreDecrypted(result);
    return result;
  };
  window.nv141RecoverE2ee = async function nv141RecoverE2ee(){
    try { await nv131RegisterCurrentDevice?.(false); await decryptMessagesInPlace(allMessages()); renderMessagesOnly(false); toast("E2EE recovery проверен"); } catch(error) { toast("E2EE recovery: " + (error.message || error)); }
  };

  // Forwarding fix: include file id extracted from URL and force server-side attachment copy when needed.
  const baseBuildPayload141 = buildOutgoingMessagePayload;
  window.buildOutgoingMessagePayload = buildOutgoingMessagePayload = async function nv141BuildOutgoingMessagePayload(chatId, text, attachment){
    return baseBuildPayload141(chatId, text, nv141AttachmentForForward(attachment));
  };
  if (typeof forwardOne139 === "function") {
    window.forwardOne139 = forwardOne139 = async function nv141ForwardOne(targetChatId, m){
      const sourceAttachment = nv141AttachmentForForward(messagePayloadAttachment(m));
      const text = messagePayloadText(m);
      const label = m.from && m.from !== S.user?.username ? `Переслано от @${m.from}\n` : "";
      const payload = await buildOutgoingMessagePayload(targetChatId, label + text, sourceAttachment || null);
      const response = await api("/chats/" + encodeURIComponent(targetChatId) + "/messages", { method:"POST", body:JSON.stringify({ ...payload, forwardedFrom:m.from || null, originalMessageId:m.id || null, ttl:0 }) });
      await decryptMessage(response.message);
      S.messages[targetChatId] = S.messages[targetChatId] || [];
      if (!S.messages[targetChatId].some((x)=>x.id===response.message.id)) S.messages[targetChatId].push(response.message);
  ...
  // Context menu boundary 3.0.
  window.nv141PlacePopup = function nv141PlacePopup(node, x, y, pad = 12){
    if (!node) return;
    if (!node.parentNode) document.body.appendChild(node);
    window.NVActionBridge?.bind?.(node);
    requestAnimationFrame(()=>{
      const vw = window.innerWidth, vh = window.innerHeight;
      node.style.maxWidth = Math.min(380, vw - pad * 2) + "px";
      node.style.maxHeight = Math.max(220, vh - pad * 2) + "px";
      node.style.overflowY = "auto";
      const r = node.getBoundingClientRect();
      const w = Math.min(r.width, vw - pad * 2);
      const hgt = Math.min(r.height, vh - pad * 2);
      let left = x;
      let top = y;
      if (left + w + pad > vw) left = vw - w - pad;
      if (top + hgt + pad > vh) top = vh - hgt - pad;
      left = Math.max(pad, left); top = Math.max(pad, top);
      node.style.left = left + "px"; node.style.top = top + "px";
    });
  };
  const baseCtx141 = ctx;
  window.ctx = ctx = function nv141Ctx(event, id){
    baseCtx141(event, id);
    const menu = document.querySelector(".ctx");
    if (menu) nv141PlacePopup(menu, event.clientX, event.clientY);
  };

  // Group avatar upload 2.0: raw image flow with stronger client-side validation.
  window.nv135PickGroupAvatar = async function nv141PickGroupAvatar(){
    try {
      const files = await pickDomFiles({ accept:"image/png,image/jpeg,image/webp,image/gif", multiple:false });
      const file = files?.[0]; if (!file) return;
      const name = String(file.name || "avatar.png");
      if (!String(file.type || "").startsWith("image/") && !/\.(png|jpe?g|webp|gif)$/i.test(name)) throw new Error("Для аватара разрешены PNG, JPEG, WebP и GIF.");
      if (file.size > 8 * 1024 * 1024) throw new Error("Аватар не должен превышать 8 MB.");
      const form = new FormData(); form.append("file", file, name);
      const uploaded = await api("/files", { method:"POST", body:form, timeout:120000 });
      window.nv135GroupAvatarUploaded = uploaded;
      const preview = URL.createObjectURL(file);
      const box = document.getElementById("gsAvatarPreview");
      if (box) box.innerHTML = `<img src="${h(preview)}" alt="avatar"><span>${h(uploaded.name || name)}</span>`;
      toast("Аватарка группы выбрана");
    } catch(error) { toast("Аватарка группы не загружена: " + (error.message || error)); }
  };

  // Cleaner fallback avatars: never put random words inside circles.
  const baseAv141 = av;
  window.av = av = function nv141Avatar(user, cls = "avatar"){
    const raw = String(user?.displayName || user?.username || user?.title || "?").replace(/Медиа|Мед|undefined|null/gi, "").trim();
    const initials = (raw || "?").slice(0, 2).toUpperCase();
    const normalized = { ...(user || {}), displayName: initials, username: raw || initials };
    const html = String(baseAv141(normalized, cls) || "");
    return html.replace(/>Мед(?:иа)?</g, `>${h(initials)}<`).replace(/data-name="[^"]*"/g, `data-name="${h(initials)}"`);
  };

  // Changelog Window 2.0.
  window.showChangelogModal = showChangelogModal = function nv141ChangelogModal(data = {}){
    const items = data.changes || data.items || data.highlights || [
      "E2EE Recovery Layer 2.0 и восстановление истории после перезапуска.",
      "Legacy DB Import 2.0 и подробный отчёт импорта backup.",
      "Исправлена пересылка голосовых/файлов в Избранное и другие чаты.",
      "Исправлена загрузка аватарок групп и fallback-аватарки.",
      "UI Spacing Audit: кнопки и элементы больше не липнут друг к другу.",
      "Context Menu Boundary Fix 3.0: ПКМ-меню не уходит за окно.",
      "Theme Consistency Fix: окна и модалки адаптируются под выбранную тему.",
      "Mention Chips Fix: @all, @admins и @username читаются на любой теме.",
      "Voice Player Stability 2.0 и сохранение состояния голосовых.",
      "Исправлен выбор микрофона и закрытие emoji-panel."
    ];
    setTimeout(()=>modal(`<div class="updateHero nv141Changelog"><div class="updateGlow">1.4</div><h2>${h(data.title || "NightVault 1.4.4 — Stability, UX Polish & E2EE Recovery Fix Update")}</h2><div class="fileCard"><b>Версия ${h(data.version || VERSION)}</b></div><ul class="changeList">${items.map((x)=>`<li>${h(x)}</li>`).join("")}</ul><button class="btn" onclick="closeModal()">Понятно</button></div>`),500);
  };
})();

/* NightVault 1.4.4 — Public Beta Hardening & Architecture Update */
(function nv142PublicBetaHardening(){
  const VERSION = "1.4.4";
  try { localStorage.nvAppVersion = VERSION; } catch {}
  if (typeof S === "object") S.appVersion = VERSION;

  window.nv142OpenE2eeRecovery = function nv142OpenE2eeRecovery(){
    const missing = allMessages().filter((m)=>m?.decryptError || /^encrypted-/i.test(String(m?.text || m?.attachment?.name || ""))).length;
    modal(`<section class="settingsPanel nv142Recovery"><h2>E2EE Recovery Layer 2.1</h2><p class="muted">Если история после перезахода не открывается, проверь устройство, chat key, shared vault и recovery bundle.</p><div class="settingCards"><div class="settingCard"><b>Device key</b><span>${h(S.e2ee?.deviceId || 'не создан')}</span><small>текущее устройство</small></div><div class="settingCard"><b>Encrypted placeholders</b><span>${missing}</span><small>сообщений требуют ключи</small></div><div class="settingCard"><b>Backup Compatibility Matrix</b><span>1.3.6–1.4.4</span><small>.nvbackup / json / sqlite dry-run</small></div></div><div class="buttonRow"><button class="btn" onclick="nv141RecoverE2ee?.();closeModal()">Проверить ключи</button><button class="btn ghost" onclick="nv131RegisterCurrentDevice?.(false);toast('Запрошена пересинхронизация E2EE')">Запросить пересинхронизацию</button><button class="btn ghost" onclick="nv120ExportRecovery?.()">Экспорт recovery</button><button class="btn ghost" onclick="localStorage.nv142HideEncrypted=localStorage.nv142HideEncrypted==='1'?'0':'1';toast('Скрывать encrypted placeholders: '+(localStorage.nv142HideEncrypted==='1'?'да':'нет'))">Скрыть encrypted placeholders</button></div></section>`);
  };

  window.nv142CopyTesterReport = async function nv142CopyTesterReport(){ try { await navigator.clipboard?.writeText(window.nv142LastTesterReport || ''); toast('Отчёт скопирован'); } catch { toast('Не удалось скопировать отчёт'); } };

  window.nv142TesterMode = async function nv142TesterMode(){
    const info = await nv?.serverInfo?.().catch(()=>null);
    const report = { version:VERSION, user:S.user?.username||'', server:getServerHttp(), runtime:info?.userDataPath||'', activeChat:S.active||'', reconnect:S.wsConnected?'online':'offline', time:new Date().toISOString() };
    window.nv142LastTesterReport = JSON.stringify(report, null, 2);
    modal(`<section class="settingsPanel"><h2>Public Tester Mode</h2><p class="muted">Быстрый отчёт для тестеров: server URL, версия, ping/reconnect и debug report.</p><pre class="testResult">${h(window.nv142LastTesterReport)}</pre><div class="buttonRow"><button class="btn" onclick="nv142CopyTesterReport()">Скопировать отчёт</button><button class="btn ghost" onclick="nv120CollectDebugReport()">Создать debug report</button><button class="btn ghost" onclick="checkUpdates()">Проверить обновления</button></div></section>`);
  };

  window.nv142TestMicrophone = async function nv142TestMicrophone(){
    try {
      const deviceId = localStorage.nvSelectedMicId || undefined;
      ...
      const level = Math.round([...data].reduce((a,b)=>a+b,0) / Math.max(1,data.length));
      stream.getTracks().forEach((t)=>t.stop());
      await ctx.close().catch(()=>{});
      toast(`Микрофон доступен. Уровень: ${level}`);
    } catch(error) { toast("Нет доступа к микрофону. Проверь разрешения Windows: " + (error.message || error)); }
  };

  const baseSettingsPanel142 = settingsPanel;
  window.settingsPanel = settingsPanel = function nv142SettingsPanel(){
    const section = S.settingsSection || localStorage.nvSettingsSection || "overview";
    if (section === "notifications") {
      const devices = S.audioDevices || [];
      return `<section class="settingsPanel"><h2>Уведомления и Voice Recorder 2.1</h2><p class="muted">Микрофон, проверка уровня, звуки, DND и Notification Center 2.1.</p><label>Микрофон<select id="micSelect" class="field nvNativeControl">${devices.map((d)=>`<option value="${h(d.deviceId)}" ${localStorage.nvSelectedMicId===d.deviceId?'selected':''}>${h(d.label||'Микрофон')}</option>`).join('') || '<option value="">Нажми “Обновить список”</option>'}</select></label><div class="buttonRow"><button class="btn" onclick="localStorage.nvSelectedMicId=document.querySelector('#micSelect')?.value||'';toast('Микрофон сохранён')">Сохранить микрофон</button><button class="btn ghost" onclick="nv141RefreshMicrophones?.()">Обновить список</button><button class="btn ghost" onclick="nv142TestMicrophone()">Проверить микрофон</button></div><div class="settingCards"><div class="settingCard"><b>Notification Center</b><span>${(S.notifications||[]).length}</span><small>сообщения, упоминания, заявки, ошибки E2EE/файлов</small></div><div class="settingCard"><b>Do Not Disturb</b><span>${localStorage.nvDndUntil?'включён':'выключен'}</span><small>тихий режим для тестов</small></div></div></section>`;
    }
    if (section === "privacy") {
      return String(baseSettingsPanel142() || "") + `<section class="settingsPanel"><h2>Recovery / Tester Tools 1.4.4</h2><div class="buttonRow"><button class="btn" onclick="nv142OpenE2eeRecovery()">E2EE Recovery Layer 2.1</button><button class="btn ghost" onclick="nv142TesterMode()">Public Tester Mode</button></div></section>`;
    }
    if (section === "developer") {
      return `<section class="settingsPanel"><h2>Диагностика 1.4.4</h2><p class="muted">Crash Report 2.1, Public Tester Mode, release preflight и smoke checklist.</p><div class="buttonRow"><button class="btn" onclick="nv120CollectDebugReport()">Crash report</button><button class="btn ghost" onclick="nv142TesterMode()">Tester mode</button><button class="btn ghost" onclick="toast('Windows smoke: scripts\\windows-smoke-test.bat')">Windows smoke</button><button class="btn ghost" onclick="toast('Preflight: npm run release:preflight')">Release preflight</button></div><textarea id="debugReport" class="field" rows="8" placeholder="Здесь появится debug report"></textarea></section>`;
    }
    return String(baseSettingsPanel142() || "").replace(/1\.4\.1/g, VERSION);
  };

  const baseCtx142 = ctx;
  window.ctx = ctx = function nv142Ctx(event, id){
    baseCtx142(event, id);
    const menu = document.querySelector('.ctx');
    if (menu) (window.NV142ContextMenu?.placeWithinViewport || window.nv141PlacePopup)?.(menu, event.clientX, event.clientY);
  };

  const baseShowChangelog142 = showChangelogModal;
  window.showChangelogModal = showChangelogModal = function nv142ChangelogModal(data = {}){
    const items = data.changes || data.items || data.highlights || [
      'Renderer architecture cleanup phase 1: voice, media viewer, context menu and UI helpers prepared as modules.',
      'UI Action Router: emoji.close, message.forward, profile.open, group.avatar and other actions go through one safe router.',
      'E2EE Recovery Layer 2.1: screen with key checks, recovery bundle hints and encrypted placeholder handling.',
      'Backup Compatibility Matrix: dry-run import for .nvbackup, .nvbackup.enc, legacy JSON, server-data and SQLite direct files.',
      'Safe Update Flow: update assistant checks backup, release assets, migrations and health before server upgrade.',
      'Native Controls Cleanup: select, option, file input, range, checkbox and scrollbars follow the active theme.',
      'Voice Recorder 2.1: microphone selector, refresh, permission check and level test returned to client settings.',
      'Admin Logs 3.0, Command Help 3.0, Server Health Alerts and Radmin Helper 3.0 for public beta testing.',
      'Installer / Release Hardening 2.0: npm run release:preflight and Windows smoke-test script.',
      'Public Tester Mode: quick debug report, server URL, version, reconnect status and tester diagnostics.'
    ];
    setTimeout(()=>modal(`<div class="updateHero nv142Changelog"><div class="updateGlow">1.4</div><h2>${h(data.title || 'NightVault 1.4.4 — Public Beta Hardening & Architecture Update')}</h2><div class="fileCard"><b>Версия ${h(data.version || VERSION)}</b></div><ul class="changeList">${items.map((x)=>`<li>${h(x)}</li>`).join('')}</ul><button class="btn" onclick="closeModal()">Понятно</button></div>`),500);
  };
})();


/* NightVault 1.4.4 — Real UX, Reliability & Tester Feedback Update */
(function nv143RealUxReliability(){
  const VERSION = "1.4.4";
  window.nv143RuntimeCheck = function nv143RuntimeCheck(){
    const checks = [
      ['app mounted', Boolean(document.querySelector('#app'))],
      ['emoji close action', Boolean(window.NV143Actions?.has?.('emoji.close'))],
      ['context menu bounds', Boolean(window.NvContextMenu?.placeWithinViewport)],
      ['voice module 2.2', Boolean(window.NV143Voice?.testMicrophone)],
      ['media viewer 2.2', Boolean(window.NV143Media?.open)],
      ['native controls css', Boolean(getComputedStyle(document.documentElement).getPropertyValue('--accent'))],
    ];
    const html = checks.map(([name, ok])=>`<div class="checkLine ${ok?'ok':'warn'}">${ok?'✅':'⚠'} ${h(name)}</div>`).join('');
    modal(`<section class="settingsPanel"><h2>Real UI Runtime Check</h2><p class="muted">Проверка живого интерфейса: emoji, ПКМ, voice/media, тема, модалки.</p>${html}<div class="buttonRow"><button class="btn" onclick="closeModal()">Готово</button><button class="btn ghost" onclick="nv143ThemePreviewLab()">Theme Preview Lab</button></div></section>`);
    return checks;
  };
  window.nv143OpenE2eeChatStatus = function nv143OpenE2eeChatStatus(){
    const failed = allMessages().filter((m)=>String(m.decryptedText || m.text || '').startsWith('encrypted-') || m.decryptError).length;
    modal(`<section class="settingsPanel nv143E2eeStatus"><h2>E2EE Recovery 2.2</h2><p class="muted">Диагностика ключей по текущему чату: устройство, chat key, shared vault, recovery cache.</p><div class="settingCards"><div class="settingCard"><b>Device</b><span>${h(S.e2ee?.deviceId || 'не создан')}</span><small>текущее устройство</small></div><div class="settingCard"><b>Chat</b><span>${h(S.active || 'нет')}</span><small>активный чат</small></div><div class="settingCard"><b>Encrypted placeholders</b><span>${failed}</span><small>сообщений требуют ключи</small></div></div><div class="buttonRow"><button class="btn" onclick="nv141RecoverE2ee?.();toast('Повторная проверка ключей запущена')">Проверить ключи этого чата</button><button class="btn ghost" onclick="nv131RegisterCurrentDevice?.(false);toast('Запрошена пересинхронизация')">Запросить пересинхронизацию</button><button class="btn ghost" onclick="localStorage.nv142HideEncrypted=localStorage.nv142HideEncrypted==='1'?'0':'1';toast('Encrypted placeholders: '+(localStorage.nv142HideEncrypted==='1'?'скрыты':'показаны'))">Скрыть encrypted placeholders</button></div></section>`);
  };
  window.nv143BackupAssistant = function nv143BackupAssistant(){
    modal(`<section class="settingsPanel nv143BackupAssistant"><h2>Backup / Import Assistant 2.2</h2><ol class="nv143Steps"><li>Выбрать backup</li><li>Проверить формат</li><li>Показать preview</li><li>Проверить совместимость</li><li>Создать safety backup</li><li>Импортировать</li><li>Применить миграции</li><li>Показать отчёт</li></ol><div class="buttonRow"><button class="btn" onclick="toast('Dry-run запускается в админке: Backups → Проверить без импорта')">Проверить без импорта</button><button class="btn ghost" onclick="toast('.nvbackup / .nvbackup.enc / legacy json / sqlite db поддерживаются')">Форматы</button><button class="btn ghost" onclick="closeModal()">Закрыть</button></div></section>`);
  };
  window.nv143ThemePreviewLab = function nv143ThemePreviewLab(){
    modal(`<section class="settingsPanel nv143ThemeLab"><h2>Theme Preview Lab</h2><p class="muted">Проверка темы на кнопках, inputs, select, cards, modals, message bubbles, voice player, emoji, context menu, toasts.</p><div class="nv143PreviewGrid"><div class="nv-card"><b>Card</b><p>Текст, muted, border, shadow.</p><button class="btn">Primary</button><button class="btn ghost">Ghost</button></div><div class="nv-card"><label>Select<select class="field nv-select"><option>Crimson</option><option>Aurora</option></select></label><label>Input<input class="field nv-input" value="NightVault"></label></div><div class="msg mine"><div class="bubble">Message bubble <span class="mention">@all</span><div class="msgTime">12:34 ✓✓</div></div></div><div class="voiceMsg nv143Voice"><button class="btn small">▶</button><div class="voiceWave"><i></i><i></i><i></i><i></i></div><span>00:08</span></div></div><div class="buttonRow"><button class="btn" onclick="closeModal()">Готово</button></div></section>`);
  };
  window.nv143CopyModalTextarea = function nv143CopyModalTextarea(){ const text=document.querySelector('.modal textarea')?.value||''; navigator.clipboard?.writeText(text).catch(()=>{}); toast('Отчёт скопирован'); };
  window.nv143TesterReport = function nv143TesterReport(){
    const report = { version: VERSION, server:getServerHttp(), user:S.user?.username || '', activeChat:S.active || '', lastError:window.__nvLastError || '', uiActions:window.NV143Actions?.debug?.slice(-20) || [], e2ee:{ device:S.e2ee?.deviceId || '', failedDecrypt:allMessages().filter((m)=>m.decryptError).length }, voice: window.NV143Voice ? { selectedMic:localStorage.nvSelectedMicId || '', players: window.NV143Voice.registry?.size || 0 } : {}, theme:S.theme, time:new Date().toISOString() };
    modal(`<section class="settingsPanel"><h2>Tester Debug Report</h2><p class="muted">Скопируй этот отчёт и отправь разработчику вместе со скрином бага.</p><textarea class="field" rows="14">${h(JSON.stringify(report,null,2))}</textarea><div class="buttonRow"><button class="btn" onclick="nv143CopyModalTextarea()">Скопировать</button><button class="btn ghost" onclick="closeModal()">Закрыть</button></div></section>`);
  };
  window.nv143OpenMediaLab = function nv143OpenMediaLab(){
    modal(`<section class="settingsPanel"><h2>Media Viewer 2.2</h2><p class="muted">Поддержка blob:, data:, /api/files, пересланных вложений и Saved Messages. Клавиши: Esc, ←, →, double click zoom.</p><div class="buttonRow"><button class="btn" onclick="toast('Открой фото из чата для полноценной проверки')">Проверить в чате</button><button class="btn ghost" onclick="closeModal()">Закрыть</button></div></section>`);
  };
  window.nv143TestMicrophone = async function nv143TestMicrophone(){
    try { const out = await window.NV143Voice?.testMicrophone?.(localStorage.nvSelectedMicId || ''); toast(`Микрофон доступен. Уровень: ${out?.level ?? 0}`); }
    catch(error) { toast('Нет доступа к микрофону. Проверь разрешения Windows: ' + (error.message || error)); }
  };
  const baseSettingsPanel143 = settingsPanel;
  window.settingsPanel = settingsPanel = function nv143SettingsPanel(){
    const section = S.settingsSection || localStorage.nvSettingsSection || 'overview';
    if (section === 'developer') {
      return `<section class="settingsPanel"><h2>Диагностика 1.4.4</h2><p class="muted">Runtime UI check, Tester Mode 2.0, native controls audit, e2e tester flows и release preflight.</p><div class="buttonRow"><button class="btn" onclick="nv143RuntimeCheck()">Real UI Runtime Check</button><button class="btn ghost" onclick="nv143TesterReport()">Tester debug report</button><button class="btn ghost" onclick="nv143ThemePreviewLab()">Theme Preview Lab</button><button class="btn ghost" onclick="nv143OpenE2eeChatStatus()">E2EE status</button><button class="btn ghost" onclick="nv143BackupAssistant()">Backup assistant</button></div><textarea id="debugReport" class="field" rows="8" placeholder="Здесь появится debug report"></textarea></section>`;
    }
    if (section === 'notifications') {
      const devices = S.audioDevices || [];
      return `<section class="settingsPanel"><h2>Voice Recorder / Player 2.2</h2><p class="muted">Выбор микрофона, тест уровня, preview перед отправкой и стабильное восстановление player.</p><label>Микрофон<select id="micSelect" class="field nvNativeControl">${devices.map((d)=>`<option value="${h(d.deviceId)}" ${localStorage.nvSelectedMicId===d.deviceId?'selected':''}>${h(d.label||'Микрофон')}</option>`).join('') || '<option value="">Нажми “Обновить список”</option>'}</select></label><div class="buttonRow"><button class="btn" onclick="localStorage.nvSelectedMicId=document.querySelector('#micSelect')?.value||'';toast('Микрофон сохранён')">Сохранить микрофон</button><button class="btn ghost" onclick="nv141RefreshMicrophones?.()">Обновить устройства</button><button class="btn ghost" onclick="nv143TestMicrophone()">Проверить микрофон</button><button class="btn ghost" onclick="nv143TesterReport()">Tester report</button></div></section>`;
    }
    return String(baseSettingsPanel143() || '').replace(/1\.4\.2/g, VERSION);
  };
  const baseCtx143 = ctx;
  window.ctx = ctx = function nv143Ctx(event, id){
    baseCtx143(event, id);
    const menu = document.querySelector('.ctx,.nvContextMenu');
    if (menu) (window.NvContextMenu?.placeWithinViewport || window.NV143ContextMenu?.placeWithinViewport)?.(menu, event.clientX, event.clientY);
  };
  const baseShowChangelog143 = showChangelogModal;
  window.showChangelogModal = showChangelogModal = function nv143ChangelogModal(data = {}){
    const items = data.changes || data.items || data.highlights || [
      'Real UI Runtime Check: проверка живого интерфейса, emoji, ПКМ, settings, profile и media viewer.',
      'Client/Admin visual polish: единые отступы, adaptive bubble layout, меньше overflow и белых native controls.',
      'Voice Recorder / Player 2.2: микрофон выбирается, тестируется и не ломает composer при ошибке Windows permissions.',
      'Media Viewer 2.2: next/prev, keyboard navigation, blob/data/api files, пересланные вложения и Saved Messages.',
      'Saved Messages 2.2 и Forwarding 2.2: preview, теги, фильтры и надёжное копирование attachment.',
      'E2EE Recovery 2.2: диагностика ключей по чату и понятные действия для восстановления истории.',
      'Backup / Import Assistant 2.2: dry-run, preview совместимости и безопасный импорт.',
      'Admin Dashboard 3.1, Logs 3.1 и Radmin Helper 3.1: быстрее диагностика сервера и подключение тестеров.',
      'Theme Preview Lab и Native Controls Final Cleanup: темы проверяются на кнопках, inputs, select, cards, tables, bubbles.',
      'UI Action Router 2.0, NvContextMenu 4.0, Notification Center 2.2 и Real E2E Tester Flows.'
    ];
    setTimeout(()=>modal(`<div class="updateHero nv143Changelog"><div class="updateGlow">1.4</div><h2>${h(data.title || 'NightVault 1.4.4 — Real UX, Reliability & Tester Feedback Update')}</h2><div class="fileCard"><b>Версия ${h(data.version || VERSION)}</b></div><ul class="changeList">${items.map((x)=>`<li>${h(x)}</li>`).join('')}</ul><button class="btn" onclick="closeModal()">Понятно</button></div>`),500);
  };
})();
...
/* NightVault 1.4.4 — bugfix layer: microphone persistence, safe UI actions, E2EE controls, banner/wallpaper, partial render, WAV playback. */
(function nv144BugfixLayer(){
  const BUGFIX_LABEL = "1.4.4";
  window.NV_BUGFIX_LABEL = BUGFIX_LABEL;

  function nv144SelectedMicId() {
    const select = document.getElementById("micSelect") || document.getElementById("mic");
    return String(select?.value || localStorage.nvSelectedMicId || localStorage.nvMicId || S.micId || "");
  }
  function nv144PersistMic(deviceId = nv144SelectedMicId(), notify = true) {
    const id = String(deviceId || "");
    S.micId = id;
    localStorage.nvMicId = id;
    localStorage.nvSelectedMicId = id;
    const a = document.getElementById("mic");
    const b = document.getElementById("micSelect");
    if (a) a.value = id;
    if (b) b.value = id;
    if (notify) toast(id ? "Микрофон сохранён" : "Будет использоваться системный микрофон");
    return id;
  }
  window.nv144SaveMicrophone = function nv144SaveMicrophone(){ nv144PersistMic(nv144SelectedMicId(), true); };
  window.nv144RefreshMicrophones = async function nv144RefreshMicrophones(){
    try {
      if (!navigator.mediaDevices?.enumerateDevices) throw new Error("MediaDevices API недоступен");
      let stream = null;
      ...
      S.audioDevices = devices;
      const saved = localStorage.nvSelectedMicId || localStorage.nvMicId || S.micId || "";
      const stillExists = !saved || devices.some((d)=>d.deviceId === saved);
      nv144PersistMic(stillExists ? saved : "", false);
      toast(devices.length ? `Микрофоны обновлены: ${devices.length}` : "Микрофоны не найдены");
      if (S.tab === "settings") render();
    } catch(error) { toast("Не удалось обновить микрофоны: " + (error.message || error)); }
  };
  window.nv144TestMicrophone = async function nv144TestMicrophone(){
    const wanted = nv144PersistMic(nv144SelectedMicId(), false);
    try {
      const tester = window.NV143Voice?.testMicrophone || window.NV142Voice?.testMicrophone;
      let result = null;
      if (tester) result = await tester(wanted);
      else {
        const stream = await navigator.mediaDevices.getUserMedia({ audio: wanted ? { deviceId:{ ideal:wanted } } : true });
        stream.getTracks().forEach((track)=>track.stop());
        result = { ok:true, level:0 };
      }
      toast(`Микрофон доступен. Уровень: ${result?.level ?? 0}`);
    } catch(error) {
      const msg = error?.name === "OverconstrainedError"
        ? "Выбранный микрофон больше не найден. Нажми “Обновить устройства” и выбери его заново."
        : "Нет доступа к микрофону. Проверь разрешения Windows: " + (error.message || error);
      toast(msg);
    }
  };
  const baseSaveSettings144 = saveSettings;
  window.saveSettings = saveSettings = async function nv144SaveSettings(){
    if (document.getElementById("micSelect") || document.getElementById("mic")) nv144PersistMic(nv144SelectedMicId(), false);
    const result = await baseSaveSettings144();
    if (document.getElementById("micSelect") || document.getElementById("mic")) nv144PersistMic(localStorage.nvSelectedMicId || localStorage.nvMicId || "", false);
    return result;
  };
  const baseStartVoice144 = startVoice;
  window.startVoice = startVoice = async function nv144StartVoice(){
    nv144PersistMic(nv144SelectedMicId(), false);
    return baseStartVoice144();
  };

  window.nv144ToggleBlurLock = function nv144ToggleBlurLock(){
    localStorage.nv140BlurLock = localStorage.nv140BlurLock === "1" ? "0" : "1";
    toast("Blur lock: " + (localStorage.nv140BlurLock === "1" ? "on" : "off"));
  };
  window.nv144BackupAesGcm = async function nv144BackupAesGcm(){
    try { await exportBackup(); } catch(error) { toast("Backup AES-GCM: " + (error.message || error)); }
  };
  window.nv144RegisterCurrentDevice = async function nv144RegisterCurrentDevice(){
    try {
      if (typeof nv131RegisterCurrentDevice === "function") await nv131RegisterCurrentDevice(false);
      else if (typeof ensureE2eeIdentity === "function") {
        const identity = await ensureE2eeIdentity(S.user?.username || localStorage.nvLastUser || "guest");
        await api('/devices/e2ee/current', { method:'POST', body:JSON.stringify({ e2eeDeviceId:identity.deviceId, e2eePublicKey:identity.publicKey, deviceName:navigator.userAgent }) });
      }
      toast("Запрошена пересинхронизация E2EE");
    } catch(error) { toast("E2EE resync: " + (error.message || error)); }
  };
  window.nv144ToggleEncryptedPlaceholders = function nv144ToggleEncryptedPlaceholders(){
    localStorage.nv142HideEncrypted = localStorage.nv142HideEncrypted === "1" ? "0" : "1";
    renderMessagesOnly(false);
    toast("Encrypted placeholders: " + (localStorage.nv142HideEncrypted === "1" ? "скрыты" : "показаны"));
  };
  window.nv144RecoverE2ee = async function nv144RecoverE2ee(){
    try {
      await nv144RegisterCurrentDevice();
      if (S.active) delete S.e2eeDevices[S.active];
      await decryptMessagesInPlace(allMessages());
      nv144PersistDecryptedScan();
      renderMessagesOnly(false);
      toast("Проверка ключей этого чата завершена");
    } catch(error) { toast("E2EE recovery: " + (error.message || error)); }
  };

  function nv144CacheKey(message) { return `nv144Decrypted_${S.user?.username || "guest"}_${message?.id || ""}`; }
  async function nv144DigestText(value) {
    try { const buf = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(String(value || ""))); return Array.from(new Uint8Array(buf)).map((b)=>b.toString(16).padStart(2,"0")).join(""); }
    catch { return ""; }
  }
  function nv144PayloadFromMessage(message) {
    const text = message?.decryptedText ?? (message?.decrypted ? message?.text : "");
    const attachment = message?.decryptedAttachment || null;
    if (!message?.id || (!text && !attachment)) return null;
    return { text:String(text || ""), attachment, chatId:message.chatId || S.active || "", at:Date.now() };
  }
  function nv144SaveDecrypted(message) {
    const payload = nv144PayloadFromMessage(message);
    if (!payload) return;
    try { localStorage[nv144CacheKey(message)] = JSON.stringify(payload); } catch {}
    if (message?.e2ee?.ciphertext) nv144DigestText(message.e2ee.ciphertext).then((hash)=>{ if(hash) try { localStorage[`nv144DecryptedCipher_${hash}`] = JSON.stringify(payload); } catch {} });
  }
  function nv144RestoreDecrypted(message) {
    if (!message?.id || (!message.decryptError && !String(message.text || "").startsWith("encrypted-"))) return message;
    let cached = null;
    try { cached = JSON.parse(localStorage[nv144CacheKey(message)] || localStorage[`nv141Decrypted_${S.user?.username || "guest"}_${message.id}`] || localStorage[`nv140Decrypted_${S.user?.username || "guest"}_${message.id}`] || "null"); } catch {}
    if (!cached) return message;
    message.decrypted = true;
    message.decryptError = false;
    message.decryptedText = cached.text || "";
    if (cached.attachment) message.decryptedAttachment = cached.attachment;
    message.decryptHint = "local_recovery_cache_3";
    return message;
  }
  function nv144PersistDecryptedScan() { for (const message of allMessages()) if (message?.decrypted) nv144SaveDecrypted(message); }
  window.nv144PersistDecryptedScan = nv144PersistDecryptedScan;
  const baseDecrypt144 = decryptMessage;
  window.decryptMessage = decryptMessage = async function nv144DecryptMessage(message){
    const result = await baseDecrypt144(message);
    if (result?.decrypted) nv144SaveDecrypted(result);
    else nv144RestoreDecrypted(result);
    return result;
  };
  const baseDecryptMessages144 = decryptMessagesInPlace;
  window.decryptMessagesInPlace = decryptMessagesInPlace = async function nv144DecryptMessagesInPlace(messages = []){
    const result = await baseDecryptMessages144(messages);
    for (const message of messages || []) { if (message?.decrypted) nv144SaveDecrypted(message); else nv144RestoreDecrypted(message); }
    return result;
  };

  const baseOpenChat144 = openChat;
  window.openChat = openChat = async function nv144OpenChat(id){
    await baseOpenChat144(id);
    nv144PersistDecryptedScan();
  };

  const baseChangeBanner144 = changeBanner;
  window.changeBanner = changeBanner = async function nv144ChangeBanner(){
    await baseChangeBanner144();
    if (S.user?.banner) {
      const ref = attachmentRef(S.user.banner);
      if (ref) {
        const url = fileUrl(ref) || fileUrl(S.user.banner) || S.user._bannerPreviewUrl || "";
        if (url && !S.user._bannerPreviewUrl) S.user._bannerPreviewUrl = url;
        hydrateFile(ref, { force:false }).then((u)=>{ if(u){ S.user._bannerPreviewUrl = u; if(S.tab === "profile") render(); } }).catch(()=>{});
      }
    }
  };

  const baseChooseWallpaper144 = chooseCustomWallpaper;
  window.chooseCustomWallpaper = chooseCustomWallpaper = async function nv144ChooseCustomWallpaper(){
    try { await baseChooseWallpaper144(); S.chatBg = "custom"; localStorage.nvChatBg = "custom"; applyVisualPrefs(); }
    catch(error) { toast("Фон чата не загружен: " + (error.message || error)); }
  };

  const baseSendMsg144 = sendMsg;
  window.sendMsg = sendMsg = async function nv144SendMsg(att = null){
    const wasReply = S.replyTo;
    await baseSendMsg144(att);
    if (wasReply && !S.replyTo) {
      document.querySelectorAll('.composer .reply').forEach((node)=>{ if (/Ответ на сообщение/i.test(node.textContent || "")) node.remove(); });
    }
    nv144PersistDecryptedScan();
  };

  window.nv144RenderContactsFilterOnly = function nv144RenderContactsFilterOnly(){
    const accepted = S.contacts?.accepted || [];
    const q = String(S.contactsQuery || "").toLowerCase();
    const filtered = q ? accepted.filter((item)=>(contactDisplay(item)+" "+item.user.username+" "+(item.note||"")).toLowerCase().includes(q)) : accepted;
    const input = document.getElementById('contactsLocalSearch');
    let container = document.getElementById('nv144ContactsList');
    if (!container && input) {
      const html = contactListHtml(filtered, 'accepted');
      const temp = document.createElement('div');
      temp.id = 'nv144ContactsList';
      temp.innerHTML = html;
      input.insertAdjacentElement('afterend', temp);
      return;
    }
    if (container) container.innerHTML = contactListHtml(filtered, 'accepted');
  };
  const baseContactsPage144 = contactsPage;
  window.contactsPage = contactsPage = function nv144ContactsPage(){
    const html = String(baseContactsPage144() || "").replace(/<p class=muted>1\.0\.9:[\s\S]*?<\/p>/, '<p class="muted">Контакты, заявки, избранные люди, заметки и настройки приватности.</p>');
    const inputMarker = /(<input id=contactsLocalSearch[^>]*>)([\s\S]*?)(<h2>Глобальный поиск<\/h2>)/;
    return html.replace(inputMarker, '$1<div id="nv144ContactsList">$2</div>$3');
  };
  const baseBind144 = bind;
  window.bind = bind = function nv144Bind(){
    baseBind144();
    const contactSearch = document.getElementById('contactsLocalSearch');
    if (contactSearch) {
      contactSearch.oninput = (event)=>{ S.contactsQuery = event.target.value; nv144RenderContactsFilterOnly(); };
    }
    const micSelect = document.getElementById('micSelect') || document.getElementById('mic');
    if (micSelect) {
      const saved = localStorage.nvSelectedMicId || localStorage.nvMicId || S.micId || "";
      if (saved) micSelect.value = saved;
      micSelect.onchange = () => nv144PersistMic(micSelect.value || "", false);
    }
  };

  const basePlayVoice144 = playVoice;
  window.playVoice = playVoice = async function nv144PlayVoice(event, ref){
    try {
      const button = event?.currentTarget;
      const audio = button?.audio;
      if (audio && audio.ended) audio.currentTime = 0;
      await basePlayVoice144(event, ref);
    } catch(error) {
      const message = String(error?.message || error || "");
      if (/interrupted by end of playback|end of playback|AbortError/i.test(message)) return;
      toast("Не удалось проиграть аудио: " + message);
    }
  };

  const baseSettingsPanel144 = settingsPanel;
  window.settingsPanel = settingsPanel = function nv144SettingsPanel(){
    const section = S.settingsSection || localStorage.nvSettingsSection || "overview";
    if (section === "notifications") {
      const devices = S.audioDevices || [];
      const saved = localStorage.nvSelectedMicId || localStorage.nvMicId || S.micId || "";
      const options = [`<option value="" ${!saved?'selected':''}>Системный микрофон</option>`].concat(devices.map((d,i)=>`<option value="${h(d.deviceId)}" ${saved===d.deviceId?'selected':''}>${h(d.label || `Микрофон ${i+1}`)}</option>`)).join('');
      return `<section class="settingsPanel"><h2>Voice Recorder / Player 2.3</h2><p class="muted">Выбор микрофона сохраняется между перезапусками, обновление устройств не вызывает UI-ошибку, проверка делает fallback при OverconstrainedError.</p><label>Микрофон<select id="micSelect" class="field nvNativeControl">${options}</select></label><div class="buttonRow"><button class="btn" onclick="nv144SaveMicrophone()">Сохранить микрофон</button><button class="btn ghost" onclick="nv144RefreshMicrophones()">Обновить устройства</button><button class="btn ghost" onclick="nv144TestMicrophone()">Проверить микрофон</button><button class="btn ghost" onclick="nv143TesterReport()">Tester report</button></div><div class="securityCard"><b>Сохранено:</b> ${h(saved || 'системный микрофон')}</div></section>`;
    }
    if (section === "privacy") {
      return `<section class="settingsPanel nv140SecurityPanel"><h2>Безопасность и приватность 1.4.4</h2><p class="muted">PIN, blur-lock, 2FA, recovery, доверенные устройства, блокировки и privacy settings.</p><div class="settingCards"><div class="settingCard"><b>Local PIN Lock</b><span>${nv140JsonLoad('nv140Pin',null)?'включён':'выключен'}</span><small>локальная защита приложения</small></div><div class="settingCard"><b>E2EE Trust</b><span>${h(S.e2ee?.deviceId||'device')}</span><small>Safety number / fingerprint</small></div><div class="settingCard"><b>Privacy</b><span>${h(S.user?.privacy?.avatar||'all')}</span><small>аватар, баннер, online, last seen</small></div></div><div class="buttonRow"><button class="btn" onclick="nv140PinSetup()">Включить / сменить PIN</button><button class="btn ghost" onclick="nv140LockNow()">Заблокировать сейчас</button><button class="btn ghost" onclick="nv144ToggleBlurLock()">Blur при потере фокуса</button><button class="btn ghost" onclick="nv140SafetyNumber()">Safety number</button><button class="btn ghost" onclick="nv140SecurityAudit()">Security audit</button></div><div class="securityCard"><b>Recovery</b><br>Используй recovery key и backup перед переустановкой. Сервер не хранит plain-ключи.</div></section>`;
    }
    if (section === "data") {
      return `<section class=settingsPanel><h2>Данные</h2><p class=muted>Локальные данные, backup, экспорт и подготовка отчёта для тестеров.</p><div class=buttonRow><button class="btn ghost" onclick="nv144BackupAesGcm()">Backup AES-GCM</button><button class="btn ghost" onclick="importBackup()">Импорт backup</button><button class="btn ghost" onclick="nv120ExportAllData()">Экспорт локальных данных</button><button class="btn danger" onclick="nv120ResetLocalData()">Сбросить локальные настройки</button></div></section>`;
    }
    return String(baseSettingsPanel144() || "")
      .replace(/1\.0\.9: поля профиля[\s\S]*?скрыт”.?/g, '')
      .replace(/onclick="nv141RefreshMicrophones\?\.\(\)"/g, 'onclick="nv144RefreshMicrophones()"')
      .replace(/onclick="nv143TestMicrophone\(\)"/g, 'onclick="nv144TestMicrophone()"')
      .replace(/onclick="localStorage\.nv140BlurLock[^"]+"/g, 'onclick="nv144ToggleBlurLock()"')
      .replace(/onclick="exportBackup\(\)"/g, 'onclick="nv144BackupAesGcm()"');
  };
  window.nv143OpenE2eeChatStatus = function nv144OpenE2eeChatStatus(){
    const failed = allMessages().filter((m)=>String(m.decryptedText || m.text || '').startsWith('encrypted-') || m.decryptError).length;
    modal(`<section class="settingsPanel nv143E2eeStatus"><h2>E2EE Recovery 2.3</h2><p class="muted">Диагностика ключей по текущему чату: устройство, chat key, shared vault, recovery cache.</p><div class="settingCards"><div class="settingCard"><b>Device</b><span>${h(S.e2ee?.deviceId || 'не создан')}</span><small>текущее устройство</small></div><div class="settingCard"><b>Chat</b><span>${h(S.active || 'нет')}</span><small>активный чат</small></div><div class="settingCard"><b>Encrypted placeholders</b><span>${failed}</span><small>сообщений требуют ключи</small></div></div><div class="buttonRow"><button class="btn" onclick="nv144RecoverE2ee()">Проверить ключи этого чата</button><button class="btn ghost" onclick="nv144RegisterCurrentDevice()">Запросить пересинхронизацию</button><button class="btn ghost" onclick="nv120ExportRecovery()">Экспорт recovery</button><button class="btn ghost" onclick="nv144ToggleEncryptedPlaceholders()">Скрыть encrypted placeholders</button></div></section>`);
  };

  // Remove stale profile text from already rendered DOM if it survived old panel markup.
