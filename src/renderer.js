...
function renderAuth() {
  ...
  app.innerHTML = `<div class="auth"><div class="authBox panelIn"><div class="logo">Night<span>Vault</span></div><div class="small">защищённый клиент · SQLite · E2EE · Sync 1.4.2</div>
...
function renderSide() {
  const c = currentChat();
  if (!c)
    return `<div class=sidePad><h2>NightVault</h2>${securitySummary()}<p class=muted>Версия 1.4.2</p></div>`;
  const u =
    c.type === "private"
      ? c.other
      : {
...
/* NightVault 1.4.2 — feature overlay and guarded UX */
const NV120 = {
  version: "1.3.0",
...
/* NightVault 1.4.2 — multi-client/session, E2EE resync and two-panel page fixes */
...
/* NightVault 1.4.2 — renderer hotfixes: links collector, file actions, E2EE key vault UX, voice UI, reactions, menus */
(function nv131Fixed2Layer(){
  const HOTFIX_LABEL = "1.4.2";
...
/* NightVault 1.4.2 — tester fixes: voice player, scroll stability, group avatars, media preview, updater/security polish */
(function nv133Layer(){
  const HOTFIX_LABEL = "1.4.2";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  ...
  window.settingsPanel = settingsPanel = function nv133SettingsPanel(){
    return String(baseSettingsPanel133() || "")
      ...
      .replace(/NightVault [^<]+/g, "NightVault 1.4.2");
  };
  const baseSettingsPage133 = settingsPage;
  window.settingsPage = settingsPage = function nv133SettingsPage(){
    return String(baseSettingsPage133() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.4.2</h1>`);
  };
...
/* NightVault 1.4.2 — Crimson default, Radmin admin hosting, Telegram voice and photo viewer */
(function nv134Layer(){
  const HOTFIX_LABEL = "1.4.2";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  try {
  ...
  window.settingsPanel = settingsPanel = function nv134SettingsPanel(){
    return String(baseSettingsPanel134() || "").replace(/NightVault [^<]+/g, "NightVault 1.4.2");
  };
  const baseSettingsPage134 = settingsPage;
  window.settingsPage = settingsPage = function nv134SettingsPage(){
    return String(baseSettingsPage134() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.4.2</h1>`).replace(/Настройки 1\.3\.\d+/g, "Настройки 1.4.2");
  };
...
/* NightVault 1.4.2 — live sync, styled group modals, real voice UI, contact notifications, group avatar */
(function nv135Layer(){
  const HOTFIX_LABEL = "1.4.2";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  ...
  window.settingsPanel = settingsPanel = function nv135SettingsPanel(){ return String(baseSettingsPanel135() || "").replace(/NightVault [^<]+/g, "NightVault 1.4.2"); };
  const baseSettingsPage135 = settingsPage;
  window.settingsPage = settingsPage = function nv135SettingsPage(){ return String(baseSettingsPage135() || "").replace(/<h1>Настройки<br>[^<]+<\/h1>/, `<h1>Настройки<br>1.4.2</h1>`).replace(/Настройки 1\.3\.\d+/g, "Настройки 1.4.2"); };
})();

/* NightVault 1.4.2 — quiet onboarding, stable live rendering, profile/banner/photo/voice fixes */
(function nv136Layer(){
  const HOTFIX_LABEL = "1.4.2";
  window.NV_HOTFIX_LABEL = HOTFIX_LABEL;
  ...
  window.settingsPanel = settingsPanel = function nv136SettingsPanel() {
    return String(baseSettingsPanel136() || "").replace(/NightVault 1\.2\.0 Mega Release/g, "NightVault 1.4.2").replace(/Messenger Features Update:/g, "NightVault 1.4.2:").replace(/1\.3\.5/g, "1.4.2");
  };
  const baseSettingsPage136 = settingsPage;
  window.settingsPage = settingsPage = function nv136SettingsPage() {
    return String(baseSettingsPage136() || "").replace(/1\.3\.5/g, "1.4.2").replace(/Messenger Features Update:/g, "NightVault 1.4.2:");
  };
})();

/* NightVault 1.4.2 — Messenger Features Update: partial render, WS-first live events, custom voice, media viewer */
(function nv137PerformanceUxLayer(){
  ...
  window.settingsPanel = settingsPanel = function nv137SettingsPanel() {
    return String(baseSettingsPanel137() || "").replace(/NightVault 1\.2\.0 Mega Release/g, "NightVault 1.4.2 Messenger Features Update").replace(/Mega Release:/g, "Messenger Features Update:").replace(/1\.3\.[0-6]/g, "1.4.2");
  };
  const baseSettingsPage137 = settingsPage;
  window.settingsPage = settingsPage = function nv137SettingsPage() {
    return String(baseSettingsPage137() || "").replace(/NightVault 1\.2\.0 Mega Release/g, "NightVault 1.4.2 Messenger Features Update").replace(/Mega Release:/g, "Messenger Features Update:").replace(/1\.3\.[0-6]/g, "1.4.2");
  };
})();

/* NightVault 1.4.2 — server admin events from Admin Pro */
...
/* NightVault 1.4.2 — Messenger Features Update: forward, saved messages, media history, folders, mentions, notifications, paste/upload UX */
(function nv139MessengerFeaturesLayer(){
  ...
  window.contactsPage = contactsPage = function nv139ContactsPage(){ return String(baseContactsPage139()||'').replace('1.0.9: отдельная система заявок, избранных, заметок и приватности “только контакты”.', 'Messenger Features 1.4.2: заявки, избранные, блокировки, уведомления и быстрый поиск.'); };
  ...
  window.settingsPanel = settingsPanel = function nv139SettingsPanel(){
    ...
    return String(baseSettingsPanel139()||'').replace(/NightVault 1\.3\.\d+/g,'NightVault 1.4.2').replace(/Admin Pro & Server Control Update/g,'Messenger Features Update');
  };
  const baseSettingsPage139 = settingsPage;
  window.settingsPage = settingsPage = function nv139SettingsPage(){ return String(baseSettingsPage139()||'').replace(/1\.3\.\d+/g,'1.4.2').replace(/Admin Pro & Server Control Update/g,'Messenger Features Update'); };
})();


/* NightVault 1.4.2 — Security & Messenger Pro Update: UI spacing, safe avatars, context menu bounds, E2EE cache and pro features */
(() => {
  const VERSION = "1.4.2";
  try { localStorage.nvAppVersion = VERSION; } catch {}
  ...
  function nv140HashPin(pin, salt) { return crypto.subtle.digest("SHA-256", new TextEncoder().encode(`${salt}:${pin}:NightVault-1.4.2`)).then((buf)=>Array.from(new Uint8Array(buf)).map((b)=>b.toString(16).padStart(2,"0")).join("")); }
  ...
  window.settingsPanel = settingsPanel = function nv140SettingsPanel(){
    ...
    if (section === "developer") return `<section class="settingsPanel"><h2>Диагностика 1.4.2</h2><p class="muted">Crash Report 2.0, performance-audit, security-audit и debug bundle.</p><div class="buttonRow"><button class="btn" onclick="nv120CollectDebugReport()">Crash report</button><button class="btn ghost" onclick="nv140SecurityAudit()">Security audit</button><button class="btn ghost" onclick="toast('Performance audit запускается командой npm run perf-audit')">Performance audit</button></div><textarea id="debugReport" class="field" rows="8" placeholder="Здесь появится debug report"></textarea></section>`;
  ...
  window.showChangelogModal = showChangelogModal = function nv140ChangelogModal(data={}){
    ...
    setTimeout(()=>modal(`<div class="updateHero nv140Changelog"><div class="updateGlow">1.4</div><h2>${h(data.title || 'NightVault 1.4.2 — Security & Messenger Pro Update')}</h2><div class="fileCard"><b>Версия ${h(data.version || VERSION)}</b></div><ul class="changeList">${changes || '<li>Добавлены Security & Messenger Pro функции и исправления интерфейса.</li>'}</ul><button class="btn" onclick="closeModal()">Понятно</button></div>`),700);
  };
})();
...
(function nv142PublicBetaHardening(){
  ...
  window.nv142OpenE2eeRecovery = function nv142OpenE2eeRecovery(){
    ...
    modal(`<section class="settingsPanel nv142Recovery"><h2>E2EE Recovery Layer 2.1</h2><p class="muted">Если история после перезахода не открывается, проверь устройство, chat key, shared vault и recovery bundle.</p><div class="settingCards"><div class="settingCard"><b>Device key</b><span>${h(S.e2ee?.deviceId || 'не создан')}</span><small>текущее устройство</small></div><div class="settingCard"><b>Encrypted placeholders</b><span>${missing}</span><small>сообщений требуют ключи</small></div><div class="settingCard"><b>Backup Compatibility Matrix</b><span>1.3.6–1.4.2</span><small>.nvbackup / json / sqlite dry-run</small></div></div><div class="buttonRow"><button class="btn" onclick="nv141RecoverE2ee?.();closeModal()">Проверить ключи</button><button class="btn ghost" onclick="nv131RegisterCurrentDevice?.(false);toast('Запрошена пересинхронизация E2EE')">Запросить пересинхронизацию</button><button class="btn ghost" onclick="nv120ExportRecovery?.()">Экспорт recovery</button><button class="btn ghost" onclick="localStorage.nv142HideEncrypted=localStorage.nv142HideEncrypted==='1'?'0':'1';toast('Скрывать encrypted placeholders: '+(localStorage.nv142HideEncrypted==='1'?'да':'нет'))">Скрыть encrypted placeholders</button></div></section>`);
  };
  ...
  window.showChangelogModal = showChangelogModal = function nv142ChangelogModal(data = {}){
    ...
    setTimeout(()=>modal(`<div class="updateHero nv142Changelog"><div class="updateGlow">1.4</div><h2>${h(data.title || 'NightVault 1.4.2 — Public Beta Hardening & Architecture Update')}</h2><div class="fileCard"><b>Версия ${h(data.version || VERSION)}</b></div><ul class="changeList">${items.map((x)=>`<li>${h(x)}</li>`).join('')}</ul><button class="btn" onclick="closeModal()">Понятно</button></div>`),500);
  };
})();
