...
function titlebar(){return `<div class="titlebar"><div class="brand"><span class="dot"></span><span>NightVault Server Admin</span><em>1.3.7</em></div><div class="win"><button id="min">−</button><button id="max">▢</button><button id="close">×</button></div></div>`}
...
function renderData(){
  ...
  layout(`<section class="panel"><div class="panelHead"><div><h2>Данные SQLite</h2><p class="muted">Read-only просмотр таблиц. В 1.3.7 добавлены лимит строк, фильтр без зависания и безопасный preview тяжёлых данных.</p></div><div class="rowActions"><button class="btn ghost" id="reload">Обновить</button></div></div><div class="dataLayout"><div class="tables">${buttons||'<p class="muted">Нет таблиц</p>'}</div><div class="dbMain"><div class="tableToolbar"><input id="dbSearch" class="field" placeholder="Фильтр по загруженным строкам" value="${h(state.dbQuery||'')}"><select id="dbLimit" class="field slim">${limits}</select><span class="muted">${rendered.rows.length}/${rendered.allRows.length} строк</span></div><div class="tableWrap">${rendered.body}</div></div></div></section>`);
...
/* NightVault 1.3.7 Server Admin Pro overlay */
function titlebar(){return `<div class="titlebar"><div class="brand"><span class="dot"></span><span>NightVault Server Admin</span><em>1.3.7</em></div><div class="win"><button id="min">−</button><button id="max">▢</button><button id="close">×</button></div></div>`}
function navButton(id,icon,text){return `<button class="${state.tab===id?'active':''}" onclick="switchAdminTab('${id}')">${icon} <span>${text}</span></button>`}
...
/* NightVault 1.3.7 — admin Radmin/LAN hosting labels */
...
/* NightVault 1.3.7 — live admin refresh, console commands and admin themes */
(function nv135AdminLayer(){
  ...
  titlebar = function nv135Titlebar(){return `<div class="titlebar"><div class="brand"><span class="dot"></span><span>NightVault Server Admin</span><em>1.3.7</em></div><div class="win"><button id="min">−</button><button id="max">▢</button><button id="close">×</button></div></div>`};
})();

/* NightVault 1.3.7 — quiet admin console, accent color and server data backup tools */
...
/* NightVault 1.3.7 — admin log filters, stronger console UX and backup v2 labels */
(function nv137AdminLayer(){
  const VERSION = '1.3.7';
  state.logFilter = state.logFilter || localStorage.nvAdminLogFilter || 'important';
  state.showDebugPolling = localStorage.nvAdminShowDebugPolling === '1';
  function noisy137(line){
    const text = String(line?.text || '');
    return /\[api\]\s+GET\s+\/api\/(contacts|chats(?:\/[^\s]+\/messages)?)/i.test(text) || /\[api\]\s+POST\s+\/api\/ws-ticket/i.test(text) || /\/api\/health/i.test(text);
  }
  function category137(line){
    const text = String(line?.text || '').toLowerCase();
    if (line?.level === 'error' || /error|ошиб|failed|exception/.test(text)) return 'errors';
    if (/\[auth\]|login|register|session|вход|регистрац/.test(text)) return 'auth';
    if (/message|сообщ/.test(text)) return 'messages';
    if (/contact|контакт|заявк/.test(text)) return 'contacts';
    if (/file|upload|файл/.test(text)) return 'files';
    if (/group|chat_update|owner|members|групп/.test(text)) return 'groups';
    if (/security|2fa|e2ee|trust|warn/.test(text)) return 'security';
    if (noisy137(line)) return 'debug';
    return 'users';
  }
  function visibleLogs137(){
    const filter = state.logFilter || 'important';
    return (state.logs || []).filter((line) => {
      const cat = category137(line);
      if (!state.showDebugPolling && cat === 'debug') return false;
      if (filter === 'all') return true;
      if (filter === 'important') return cat !== 'debug';
      return cat === filter;
    }).slice(-260);
  }
  function filterBtn137(id, text){ return `<button class="chip ${state.logFilter===id?'active':''}" onclick="setAdminLogFilter137('${id}')">${text}</button>`; }
  window.setAdminLogFilter137 = function(id){ state.logFilter=id; localStorage.nvAdminLogFilter=id; renderConsole(); };
  window.toggleAdminDebugLogs137 = function(){ state.showDebugPolling=!state.showDebugPolling; localStorage.nvAdminShowDebugPolling=state.showDebugPolling?'1':'0'; renderConsole(); };
  titlebar = function nv137Titlebar(){return `<div class="titlebar"><div class="brand"><span class="dot"></span><span>NightVault Server Admin</span><em>${VERSION}</em></div><div class="win"><button id="min">−</button><button id="max">▢</button><button id="close">×</button></div></div>`};
  renderConsole = function nv137RenderConsole(){
    const logs = visibleLogs137();
    const filters = `<div class="adminLogFilters">${filterBtn137('important','Важное')}${filterBtn137('all','Все')}${filterBtn137('auth','Auth')}${filterBtn137('users','Users')}${filterBtn137('messages','Messages')}${filterBtn137('contacts','Contacts')}${filterBtn137('files','Files')}${filterBtn137('groups','Groups')}${filterBtn137('security','Security')}${filterBtn137('errors','Errors')}${filterBtn137('debug','Debug')}<label class="debugToggle"><input type="checkbox" ${state.showDebugPolling?'checked':''} onchange="toggleAdminDebugLogs137()"> polling/debug</label></div>`;
    layout(`<section class="panel"><div class="panelHead"><div><h2>Консоль сервера</h2><p class="muted">1.3.7: фильтры логов, скрытый polling-шум и расширенные команды.</p></div><div class="rowActions"><button class="btn ghost" id="refresh">Обновить</button><button class="btn ghost" id="clearLocal">Очистить вид</button></div></div>${filters}<div class="console" id="console">${logs.map(logLine).join('')}</div><div class="adminCommand"><input id="adminCmd" class="field" placeholder="help, stats, online, users, info user test, devices test, chats test, logs errors, backup create"><button class="btn" id="runCmd">Выполнить</button></div><pre id="cmdOut" class="testResult">Введите help для списка команд NightVault Admin 1.3.7.</pre></section>`);
    $('#refresh').onclick=async()=>{await refreshStatus();renderConsole();};
    $('#clearLocal').onclick=()=>{state.logs=[];renderConsole();};
    $('#runCmd').onclick=runAdminCommand137;
    $('#adminCmd').addEventListener('keydown',e=>{if(e.key==='Enter')runAdminCommand137();});
    setTimeout(()=>{$('#console')?.scrollTo(0,999999)},0);
  };
  async function runAdminCommand137(){
    const cmd = $('#adminCmd')?.value || '';
    const out = $('#cmdOut');
    if(out) out.textContent = 'Выполняю: ' + cmd;
    try{ const r = await nvAdmin.command(cmd); if(out) out.textContent = r?.text || JSON.stringify(r?.data ?? r, null, 2); await refreshStatus(); }
  ...
  const baseSettings137 = renderSettings;
  renderSettings = function nv137RenderSettings(){
    baseSettings137();
    const panel = document.querySelector('.adminSettings136, .settingsForm');
    if(panel && !panel.querySelector('.nv137BackupNote')){
      const note = document.createElement('div');
      note.className='securityCard nv137BackupNote';
      note.innerHTML='<b>Backup package v2</b><br>Экспорт создаёт <code>.nvbackup</code> с manifest, database, uploads и checksum. Перед импортом автоматически создаётся rollback backup.';
