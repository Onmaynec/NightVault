...
function titlebar(){return `<div class="titlebar"><div class="brand"><span class="dot"></span><span>NightVault Server Admin</span><em>1.3.5</em></div><div class="win"><button id="min">−</button><button id="max">▢</button><button id="close">×</button></div></div>`}
...
function renderData(){
  ...
  layout(`<section class="panel"><div class="panelHead"><div><h2>Данные SQLite</h2><p class="muted">Read-only просмотр таблиц. В 1.3.5 добавлены лимит строк, фильтр без зависания и безопасный preview тяжёлых данных.</p></div><div class="rowActions"><button class="btn ghost" id="reload">Обновить</button></div></div><div class="dataLayout"><div class="tables">${buttons||'<p class="muted">Нет таблиц</p>'}</div><div class="dbMain"><div class="tableToolbar"><input id="dbSearch" class="field" placeholder="Фильтр по загруженным строкам" value="${h(state.dbQuery||'')}"><select id="dbLimit" class="field slim">${limits}</select><span class="muted">${rendered.rows.length}/${rendered.allRows.length} строк</span></div><div class="tableWrap">${rendered.body}</div></div></div></section>`);
...
/* NightVault 1.3.5 Server Admin Pro overlay */
function titlebar(){return `<div class="titlebar"><div class="brand"><span class="dot"></span><span>NightVault Server Admin</span><em>1.3.5</em></div><div class="win"><button id="min">−</button><button id="max">▢</button><button id="close">×</button></div></div>`}
function navButton(id,icon,text){return `<button class="${state.tab===id?'active':''}" onclick="switchAdminTab('${id}')">${icon} <span>${text}</span></button>`}
...
/* NightVault 1.3.5 — admin Radmin/LAN hosting labels */
...
/* NightVault 1.3.5 — live admin refresh, console commands and admin themes */
(function nv135AdminLayer(){
  ...
  titlebar = function nv135Titlebar(){return `<div class="titlebar"><div class="brand"><span class="dot"></span><span>NightVault Server Admin</span><em>1.3.5</em></div><div class="win"><button id="min">−</button><button id="max">▢</button><button id="close">×</button></div></div>`};
})();
