...
async function startAdminServer() {
  ...
  if (await probeNightVaultServer(host, preferredPort)) {
    adminServerUrl = `http://${host}:${preferredPort}`;
  ...
  if (adminServerModule.server.listening) {
    ...
    adminServerUrl = `http://${host}:${port}`;
    adminServerStatus = { ok: true, mode: "embedded", url: adminServerUrl, message: "Сервер уже работает внутри админ-приложения." };
    return adminServerStatus;
  }
  ...
  adminServerUrl = `http://${host}:${port}`;
  adminServerStatus = { ok: true, mode: "embedded", url: adminServerUrl, message: port === preferredPort ? "Сервер запущен." : `Порт ${preferredPort} был занят, сервер запущен на ${port}.` };
...
async function runAdminTest(name) {
  ...
  const probe = async () => {
    ...
    try { return await probeNightVaultServer("127.0.0.1", Number(new URL(status.url).port)); }
    catch { return false; }
  };
...
handle("admin-login", (_event, payload = {}) => {
  ...
  return { ok: true, username: "admin", version: String(app.getVersion() || "").includes("globalfix") ? "1.3.3" : app.getVersion() };
});
...
handle("admin-status", () => ({ authenticated: adminAuthenticated, server: adminServerStatus, url: adminServerUrl, version: String(app.getVersion() || "").includes("globalfix") ? "1.3.3" : app.getVersion() }));
...
handle("app-version", () => { const v = app.getVersion() || "1.3.3"; return String(v).includes("globalfix") ? "1.3.3" : v; });
