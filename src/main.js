...
async function exportServerDataBundle() {
  ...
  const result = await dialog.showSaveDialog(adminWindow || mainWindow, {
    ...
    defaultPath: `nightvault-server-data-${app.getVersion() || "1.4.3"}-${new Date().toISOString().slice(0,10)}.json`,
    filters: [{ name: "NightVault server data", extensions: ["json"] }],
  });
  if (result.canceled || !result.filePath) return { ok: false, canceled: true };
  const bundle = { format: "nightvault-server-data", version: app.getVersion() || "1.4.3", exportedAt: new Date().toISOString(), db: cloneForExport(store.db), uploads: collectUploadBundle() };
...
function summarizeImportedBundleForPreview(bundle, sourcePath = "") {
  ...
  const current = app.getVersion() || "1.4.3";
...
async function importServerDataBundle() {
  ...
  const raw = JSON.parse(text);
  if (String(filePath).endsWith(".enc") || raw?.encrypted === true || raw?.format === "nightvault-backup-encrypted") {
    throw new Error("Зашифрованный backup найден. В 1.4.3 импорт .enc делается через Backup Manager с паролем, чтобы не потерять данные.");
  }
  const bundle = normalizeImportedServerBundle(raw, filePath);
...
handle("admin-login", (_event, payload = {}) => {
  ...
  return { ok: true, username: "admin", version: String(app.getVersion() || "").includes("globalfix") ? "1.4.3" : app.getVersion() };
});
...
handle("admin-status", () => ({ authenticated: adminAuthenticated, server: adminServerStatus, url: adminServerUrl, version: String(app.getVersion() || "").includes("globalfix") ? "1.4.3" : app.getVersion() }));
...
handle("app-version", () => { const v = app.getVersion() || "1.4.3"; return String(v).includes("globalfix") ? "1.4.3" : v; });

/* NightVault 1.4.3 — Admin Pro commands and safe backup package v2 */
(function nv137MainLayer(){
  ...
  runAdminConsoleCommand = function nv137RunAdminConsoleCommand(commandText) {
    ...
    if (!command || cmd === "help" || cmd === "?") return { ok:true, text:[
      "NightVault Admin 1.4.3 commands:",
    ...
    if (["ban", "kick", "reset2fa", "restore", "delete"].includes(cmd)) return { ok:false, text:"Опасная команда отключена в 1.4.3. Нужен отдельный экран подтверждения." };
    return baseRunAdminCommand137(commandText);
  };
})();

/* NightVault 1.4.3 — Admin Pro & Server Control layer */
...
/* NightVault 1.4.3 — legacy backup import compatibility and safer admin import */
