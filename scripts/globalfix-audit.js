...
check("package semver 1.3.9", pkg.version === "1.3.9", pkg.version);
check("UI release label", renderer.includes("1.3.9") && adminRenderer.includes("1.3.9"));
...
check("1.3.9 optimistic avatar preview", renderer.includes("setTemporaryProfilePreview") && renderer.includes("_avatarPreviewUrl") && renderer.includes("assetUploading.avatar"));
check("1.3.9 chat bottom observer", renderer.includes("ensureChatBottomWatch") && renderer.includes("MutationObserver") && renderer.includes("ResizeObserver"));
check("1.3.9 admin row limit", adminRenderer.includes("tableLimit") && adminRenderer.includes("dbLimit") && adminCss.includes(".dbMain"));
check("1.3.9 UI cleanup css", style.includes("NightVault 1.3.9") && style.includes("chatEmptyState") && style.includes("uploadNotice"));
...
if (failed.length) {
  console.error(`NightVault 1.3.9 audit failed: ${failed.length} checks failed.`);
  process.exit(1);
}
console.log(`NightVault 1.3.9 audit passed: ${checks.length} checks.`);
