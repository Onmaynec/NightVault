...
check("package semver 1.4.3", pkg.version === "1.4.3", pkg.version);
check("UI release label", renderer.includes("1.4.3") && adminRenderer.includes("1.4.3"));
...
check("1.4.3 optimistic avatar preview", renderer.includes("setTemporaryProfilePreview") && renderer.includes("_avatarPreviewUrl") && renderer.includes("assetUploading.avatar"));
check("1.4.3 chat bottom observer", renderer.includes("ensureChatBottomWatch") && renderer.includes("MutationObserver") && renderer.includes("ResizeObserver"));
check("1.4.3 admin row limit", adminRenderer.includes("tableLimit") && adminRenderer.includes("dbLimit") && adminCss.includes(".dbMain"));
check("1.4.3 UI cleanup css", style.includes("NightVault 1.4.3") && style.includes("chatEmptyState") && style.includes("uploadNotice"));
...
if (failed.length) {
  console.error(`NightVault 1.4.3 audit failed: ${failed.length} checks failed.`);
  process.exit(1);
}
console.log(`NightVault 1.4.3 audit passed: ${checks.length} checks.`);
