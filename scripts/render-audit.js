...
check("release version is 1.3.7", pkg.version === "1.3.7", pkg.version);
check("renderer release label is 1.3.7", renderer.includes('const RELEASE_LABEL = "1.3.7"'));
check("admin titlebar label is 1.3.7", admin.includes("1.3.7"));
...
check("1.3.7 optimistic avatar/banner preview", renderer.includes("setTemporaryProfilePreview") && renderer.includes("settleTemporaryProfilePreview") && renderer.includes("assetDisplayUrl"));
check("1.3.7 bottom lock observer", renderer.includes("ensureChatBottomWatch") && renderer.includes("chatDistanceFromBottom") && renderer.includes("S.chatPinnedBottom"));
check("1.3.7 chat empty state", renderer.includes("chatEmptyState") && style.includes(".chatEmptyState"));
check("1.3.7 admin limit toolbar", admin.includes("dbLimit") && adminCss.includes(".dbMain") && adminCss.includes(".miniLoader"));
