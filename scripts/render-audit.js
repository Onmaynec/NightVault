...
check("release version is 1.4.2", pkg.version === "1.4.2", pkg.version);
check("renderer release label is 1.4.2", renderer.includes('const RELEASE_LABEL = "1.4.2"'));
check("admin titlebar label is 1.4.2", admin.includes("1.4.2"));
...
check("1.4.2 optimistic avatar/banner preview", renderer.includes("setTemporaryProfilePreview") && renderer.includes("settleTemporaryProfilePreview") && renderer.includes("assetDisplayUrl"));
check("1.4.2 bottom lock observer", renderer.includes("ensureChatBottomWatch") && renderer.includes("chatDistanceFromBottom") && renderer.includes("S.chatPinnedBottom"));
check("1.4.2 chat empty state", renderer.includes("chatEmptyState") && style.includes(".chatEmptyState"));
check("1.4.2 admin limit toolbar", admin.includes("dbLimit") && adminCss.includes(".dbMain") && adminCss.includes(".miniLoader"));
