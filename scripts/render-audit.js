...
check("release version is 1.3.6", pkg.version === "1.3.6", pkg.version);
check("renderer release label is 1.3.6", renderer.includes('const RELEASE_LABEL = "1.3.6"'));
check("admin titlebar label is 1.3.6", admin.includes("1.3.6"));
...
check("1.3.6 optimistic avatar/banner preview", renderer.includes("setTemporaryProfilePreview") && renderer.includes("settleTemporaryProfilePreview") && renderer.includes("assetDisplayUrl"));
check("1.3.6 bottom lock observer", renderer.includes("ensureChatBottomWatch") && renderer.includes("chatDistanceFromBottom") && renderer.includes("S.chatPinnedBottom"));
check("1.3.6 chat empty state", renderer.includes("chatEmptyState") && style.includes(".chatEmptyState"));
check("1.3.6 admin limit toolbar", admin.includes("dbLimit") && adminCss.includes(".dbMain") && adminCss.includes(".miniLoader"));
