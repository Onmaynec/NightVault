...
check("release version is 1.4.1", pkg.version === "1.4.1", pkg.version);
check("renderer release label is 1.4.1", renderer.includes('const RELEASE_LABEL = "1.4.1"'));
check("admin titlebar label is 1.4.1", admin.includes("1.4.1"));
...
check("1.4.1 optimistic avatar/banner preview", renderer.includes("setTemporaryProfilePreview") && renderer.includes("settleTemporaryProfilePreview") && renderer.includes("assetDisplayUrl"));
check("1.4.1 bottom lock observer", renderer.includes("ensureChatBottomWatch") && renderer.includes("chatDistanceFromBottom") && renderer.includes("S.chatPinnedBottom"));
check("1.4.1 chat empty state", renderer.includes("chatEmptyState") && style.includes(".chatEmptyState"));
check("1.4.1 admin limit toolbar", admin.includes("dbLimit") && adminCss.includes(".dbMain") && adminCss.includes(".miniLoader"));
