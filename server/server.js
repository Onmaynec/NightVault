...
function cloneFileRecordForChat(source, chatId, username) {
  const nextId = crypto.randomUUID();
  const srcPath = path.join(config.uploadsDir, source.id);
  const dstPath = path.join(config.uploadsDir, nextId);
  try { fs.copyFileSync(srcPath, dstPath); } catch { return null; }
  const cloned = {
    ...source,
    id: nextId,
    owner: username,
    ...
    kind: "attachment",
    createdAt: now(),
    forwardedFromFileId: source.id,
  };
  db.files[nextId] = cloned;
  return cloned;
}
function bindUploadedFile(fileId, chatId, username) {
  let file = db.files[fileId];
  if (!file) return null;
  if (file.owner !== username && !canAccessFile(file, username)) return null;
  if (file.chatId && file.chatId !== chatId) {
    file = cloneFileRecordForChat(file, chatId, username);
    if (!file) return null;
  } else {
    file.chatId = chatId;
  ...
  return {
    ...
    size: file.size,
    type: file.mime,
    voice: file.fileClass === "audio" || String(file.mime || "").startsWith("audio/"),
  };
}
...
// NightVault 1.4.3 — maintenance mode and server metrics for Admin Pro.
app.use((req, res, next) => {
...
app.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const sniffed = await sniffMime(filePath, file.mime, file.originalName);
  ...
  asyncRoute(async (req, res) => {
    ...
    if (
      ...
      if (req.body.avatar !== undefined) {
        ...
        const file = db.files[fileId];
        const filePath = file ? path.join(config.uploadsDir, file.id) : "";
        const detected = file ? sniffMime(filePath, file.mime, file.originalName) : null;
        const imageLike = file && (String(detected?.mime || file.mime || "").startsWith("image/") || /\.(png|jpe?g|webp|gif)$/i.test(String(file.originalName || "")));
        if (!file || file.owner !== req.user.username || !imageLike || detected?.mime === "image/svg+xml") {
          return sendError(res, 400, "Недопустимый файл аватара. Разрешены PNG, JPEG, WebP и GIF до 8 MB.");
        }
        file.mime = detected?.mime || file.mime;
        file.kind = "avatar";
        file.chatId = chat.id;
...
// NightVault 1.4.3 Messenger Features Update: notes, links and tester diagnostics endpoints
...
// NightVault 1.4.3 — Security & Messenger Pro server layer.
app.get("/api/security/audit", auth, (req, res) => {
  const user = req.user;
  const sessions = Object.values(db.refreshTokens || {}).filter((token) => token.username === user.username);
  const devices = Object.values(user.e2eeDevices || {});
  return res.json({
    ok: true,
    username: user.username,
    ...
    untrustedDevices: devices.filter((device) => device.trusted === false).length,
    recovery: Boolean(user.e2eeRecovery),
    blockedUsers: (db.blocks?.[user.username] || []).length,
    backupEncryption: "available",
    csp: "strict-local",
    warnings: [],
  });
});
app.get("/api/privacy", auth, (req, res) => {
  res.json({ privacy: normalizePrivacy(req.user.privacy || {}) });
});
app.put("/api/privacy", auth, asyncRoute(async (req, res) => {
  req.user.privacy = normalizePrivacy({ ...(req.user.privacy || {}), ...(req.body || {}) });
  await save({ immediate: true });
  return res.json({ ok: true, privacy: req.user.privacy });
}));
app.get("/api/blocks", auth, (req, res) => {
  const list = db.blocks?.[req.user.username] || [];
  res.json({ users: list.map((username) => profileFor(username, req.user.username)).filter(Boolean) });
});
app.get("/api/reports", auth, (req, res) => {
  const mine = [];
  for (const [target, items] of Object.entries(db.reputation || {})) {
    for (const item of items || []) if (item.type === "report" && (item.from === req.user.username || req.user.username === "admin")) mine.push({ target, ...item });
  }
  res.json({ reports: mine.slice(-200).reverse() });
});
