...
// NightVault 1.4.0 — maintenance mode and server metrics for Admin Pro.
app.use((req, res, next) => {
  ...
  asyncRoute(async (req, res) => {
    ...
    if (
      ...
      if (req.body.avatar !== undefined) {
        ...
        const file = db.files[fileId];
        const imageLike = file && (String(file.mime || "").startsWith("image/") || /\.(png|jpe?g|webp|gif)$/i.test(String(file.originalName || "")));
        if (!file || file.owner !== req.user.username || !imageLike) {
...
// NightVault 1.4.0 Messenger Features Update: notes, links and tester diagnostics endpoints
...
// NightVault 1.4.0 — Security & Messenger Pro server layer.
